...
{
  SetConsoleTitle("AfterLife MMO :: Master Server");
  ...
  switch(PacketData->EventID) 
  {
    ...
    case SBPKT_S2M_GameDisconnect:
    {
      ...
      if(it != games_.end())
      {
        ...
        r3dOutToLog("!!! server %s closed unexpectedly, id:%d, super:%s, sess:%I64x\n", 
          game->GetName(), 
    ...
    case SBPKT_G2M_RegisterGame:
    {
      ...
      if(super == NULL) {
        ...
        r3dOutToLog("!!! the server 0x%x without supervisor\n", n.gameId);
        break;
      }
      ...
      {
        r3dOutToLog("!!! the server 0x%x is already connected\n", n.gameId);
      ...
      // register game
      r3dOutToLog("the server %s is connected\n", game->GetName());
