...
void CMasterServerConfig::LoadConfig()
{
  ...
  LoadPermGamesConfig();
  Temp_Load_AfterlifeGames();
  
  OnGameListUpdated();
}

void CMasterServerConfig::Temp_Load_AfterlifeGames()
{

  char group[128];
  sprintf(group, "AfterLifeGames");
  ...
  {
    GBGameInfo ginfo;
    ginfo.mapId      = GBGameInfo::MAPID_AM_PineTop;
  ...
  {
	  GBGameInfo ginfo;
	  ginfo.mapId      = GBGameInfo::MAPID_AM_Island;
	  ginfo.flags      = GBGameInfo::SFLAGS_Nameplates | GBGameInfo::SFLAGS_CrossHair | GBGameInfo::SFLAGS_Tracers;
...
static int StringToGBMapID(char* str)
{
  ...
  if(stricmp(str, "MAPID_AM_Island") == 0)
    return GBGameInfo::MAPID_AM_Island;
  if(stricmp(str, "MAPID_AM_Battleroyale") == 0)
    return GBGameInfo::MAPID_AM_Battleroyale;
  if(stricmp(str, "MAPID_AM_Compound") == 0)
    return GBGameInfo::MAPID_AM_Compound;
  if(stricmp(str, "MAPID_AM_ConventionC") == 0)
    return GBGameInfo::MAPID_AM_ConventionC;
  if(stricmp(str, "MAPID_AM_Devmap") == 0)
    return GBGameInfo::MAPID_AM_Devmap;
  if(stricmp(str, "MAPID_AM_PineTop") == 0)
    return GBGameInfo::MAPID_AM_PineTop;
  if(stricmp(str, "MAPID_AM_RoTB") == 0)
    return GBGameInfo::MAPID_AM_RoTB;
  if(stricmp(str, "MAPID_AM_Nevada") == 0)
    return GBGameInfo::MAPID_AM_Nevada;
  if(stricmp(str, "MAPID_AM_SouthWest") == 0)
    return GBGameInfo::MAPID_AM_SouthWest;
  if(stricmp(str, "MAPID_AM_RentServer01") == 0)
    return GBGameInfo::MAPID_AM_RentServer01;
  if(stricmp(str, "MAPID_AM_RentServer02") == 0)
    return GBGameInfo::MAPID_AM_RentServer02;
...
void CMasterServerConfig::ParsePermamentGame(int gameServerId, const char* name, const char* map, const char* data, int DisableASR ,int DisableSNP, int DisableWPN, int OnlyFPS)
{
  ...
  int gameType = 0;
	...
	if(8 != sscanf(data, "%d %d %d %d %d %d %d %d", &maxPlayers, &minLevel, &maxLevel, &channel, &gameTimeLimit, &GassOnMap, &isPVETheMap, &gameType)) 
	{
