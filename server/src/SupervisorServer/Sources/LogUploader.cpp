...
static	const char*	gDomainBaseUrl= "/GameAPI/app/";
...
void CFileData::DeleteFile()
{
	...
	char fullfname[MAX_PATH];
	sprintf(fullfname, "logs_SupervisorServer\\%s", fname);
...
int CLogUploader::ScanForNewLogs()
{
	...
	// scan for pictures
	h = FindFirstFile("logs_SupervisorServer\\*.jpg", &ffblk);
	...
	// scan for dumps
	h = FindFirstFile("logs_SupervisorServer\\*.dmp", &ffblk);
	if(h != INVALID_HANDLE_VALUE) do 
	{
