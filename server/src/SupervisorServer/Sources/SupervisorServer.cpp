...
void CSupervisorServer::SetColor(int ForgC)
{
	WORD wColor;
...
{
  SetConsoleTitle("AfterLife MMO :: SupervisorServer");

  _mkdir("Logs_SupervisorServer");
  time_t t1;
  time(&t1);
  char fname[MAX_PATH];
  sprintf(fname, "Logs_SupervisorServer\\SV_%x.txt", (DWORD)t1);
  extern void r3dChangeLogFile(const char* fname);
  r3dChangeLogFile(fname);

  sprintf(fname, "Logs_SupervisorServer\\SV_%x.dmp", (DWORD)t1);
...
void CSupervisorServer::Tick()
{
  ...
  if(r3dGetTime() > nextLog) 
  {
    ...
    char buf[1024];
    sprintf(buf, "AfterLife MMO :: Supervisor Server: %d num games - %2.0f%% CPU Stats %s", numGames, value.doubleValue, shuttingDown_ ? "(SHUTTING DOWN)" : "");
    SetConsoleTitle(buf);
  }
