...
//	if(gServerLogic.ginfo_.channel==6 && gServerLogic.ginfo_.mapId==GBGameInfo::MAPID_ServerTest) // public test environment server, do not save any info about player, as those maps might have game breaking loot\changes, so let's not propagate them to the rest of the game
//		updateCharEnable=false;
//#endif
...
//#endif
	...
	if(updateCharEnable)
	{
		...
		req.AddParam("ts03",        slot.Stats.VictorysHardGames);//battleroyale
		req.AddParam("ts04",        0);
		req.AddParam("ts05",        0);
...
int CJobChangeWeaponSkins::Exec()
{
	CWOBackendReq req("api_UptadeWeaponSkins.aspx");
	req.AddSessionInfo(CustomerID, SessionID);
	req.AddParam("skey1",  g_ServerApiKey);

	req.AddParam("WeaponItemID", ItemID);
	req.AddParam("Skin",  Skin);
	req.AddParam("Selected",  LastSkinUsed);

	// issue
	if(!req.Issue())
	{
		r3dOutToLog("!!!! WeaponSkinsThread failed, code: %d\n", req.resultCode_);
		return req.resultCode_;
	}
...
int CJobGetLootboxData::Exec()
{
	...
	CkHttpRequest req;
	req.UsePost();
	req.put_Path("/GameAPI/app/php/api_GetLootBoxConfig.php");
	...
	if(strcmp(api_ip, "localhost") == 0) api_ip = "25.91.238.22";
  ...
  : CAsyncApiJob(plr)
{
	...
	pugi::xml_document xmlFileGear;
	
	xml_string_writer xmlBufGear;
	xmlFileGear.save(xmlBufGear, "", pugi::format_raw);
}
