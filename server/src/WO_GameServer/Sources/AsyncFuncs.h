...
class CJobBanChatUser : public CAsyncApiJob // Ban chat
{
public:
	int		isPermaBan;
	char		BanReason[256];	
...
class CJobChangeWeaponSkins : public CAsyncApiJob
{
  public:
	int ItemID;
	int Skin;
  ...
  public:
	CJobChangeWeaponSkins(const obj_ServerPlayer* plr) : CAsyncApiJob(plr)
	{
		sprintf(desc, "CJobChangeWeaponSkins[%d] %p", CustomerID, this);
	}
