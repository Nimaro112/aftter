...
BOOL obj_Vehicle::OnCreate()
{
	...
	if (vehicleType == VEHICLETYPE_COPCAR)
	{
		maxPlayerCount = 4;
	...
	for (int i = 0; i < results; ++i)
	{
		...
		if (sweepHits[i].shape && (target = static_cast<PhysicsCallbackObject*>(sweepHits[i].shape->getActor().userData)))
		{
			...
			if ( (movementSpeed + _NetLagSpeedCompensation) > 10.0f )
			{
				...
				else if( gameObj->Class->Name == "obj_Door" )
				{
					obj_Door* Door = (obj_Door*)gameObj;
					if (Door->m_Health > 0.0f)
					{
						Door->DamageDoor( Door->m_Health + 1 );
					}
				}
...
bool obj_Vehicle::IsExitSafe(int seatPosition, r3dPoint3D& outPosition)
{
...

	{
		...
		else
		{
			...
			outPosition = r3dPoint3D(hit.impact.x, hit.impact.y + 1.5f, hit.impact.z);
...
{
	return obj_Vehicle::VEHICLETYPE_HUMMER;
}
...
void obj_Vehicle::OnNetPacket(const PKT_C2C_VehicleMoveSetCell_s& n)
{
	...
	//TheHexa:: DISABLED, SOME BUGS CAN MAKE PLAYER GET DC
...
void obj_Vehicle::OnNetPacket(PKT_C2C_VehicleMoveRel_s& n)
{
	...
	RelayPacket(&n, sizeof(n), true); //:: LETS TRY TRUE, BECAUSE VEHICLE PACKETS ITS PRIORITY!
}
