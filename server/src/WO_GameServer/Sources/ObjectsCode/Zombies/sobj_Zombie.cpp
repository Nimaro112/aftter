...
AUTOREGISTER_CLASS(obj_Zombie);
	...
	float		_zai_AttackDamage					= 15.0f;
	float		_zai_SuperZombieSuperAttackDamage	= 25.75f;	// 1.25 * _zai_AttackDamage
	int		_zai_DebugAI							= 0;
#endif
	...
	float		_zai_AttackTimer					= 1.0f;
	float		_zai_SuperZombieAttackTimer			= 1.03f;
	...
	float		_zai_ChanceOfInfecting				= 8.0f;		// 3% chance
	...
	const float _zstat_MaxHealthDogZombie = 500;
...
// 		}
	...
	else
	{
		...
		WalkSpeed   = FastZombie ? 2.3f : 2.0f;
		RunSpeed    = FastZombie ? 4.8f : 4.1f;
...
bool obj_Zombie::CheckViewToPlayer(const GameObject* obj)
{
	...
	if(!plr)
		return false;
	/*if(plr->inWounded) // wounded disabled by now
		return false;*/
	if(plr->loadout_->Alive == 0)
		return false;
...
bool obj_Zombie::SenseGrenadeSound(const obj_ServerGrenade* grenade, bool isExplosion)
{
	...
	if( isExplosion )
	{
		switch(grenade->m_ItemID)
		{
			...
			case WeaponConfig::ITEMID_smokeg_red:
			case WeaponConfig::ITEMID_smokeg_blue:
			case WeaponConfig::ITEMID_smokeg_green:
			case WeaponConfig::ITEMID_smokeg_orange:
			case WeaponConfig::ITEMID_FlashBang:
			case WeaponConfig::ITEMID_Grenade_HB:
			case WeaponConfig::ITEMID_FragGrenade_02: 

			//BURSTFIRE
			case WeaponConfig::ITEMID_FragGrenadeBF:
			case WeaponConfig::ITEMID_SmokeGrenadeBF:
			case WeaponConfig::ITEMID_MiniChargeBF:
			//BURSTFIRE
			range = 60;
				break;

			case WeaponConfig::ITEMID_IncenGrenade:
			range = 200; // testing
				break;
		}
		if(dist > range)
	...
	{
		case WeaponConfig::ITEMID_FragGrenade:
		case WeaponConfig::ITEMID_smokeg_white:
		case WeaponConfig::ITEMID_smokeg_red:
		case WeaponConfig::ITEMID_smokeg_blue:
		case WeaponConfig::ITEMID_smokeg_green:
		case WeaponConfig::ITEMID_smokeg_orange:
...
void obj_Zombie::SenseChaosExplosion(const obj_ChaosObject* chaos)
{
	if(ZombieState == EZombieStates::ZState_Dead || ZombieState == EZombieStates::ZState_Waking)
		return;

	if(!chaos)
		return;

	float dist = (GetPosition() - chaos->GetPosition()).Length();	
	if (dist > 80.0f)
		return;

	StartAttack(chaos);

	lastTargetPos = chaos->GetPosition();
	MoveNavAgent(chaos->GetPosition(), _zai_MaxPursueDistance);
}
...
bool obj_Zombie::CanAttack(const GameObject* trg)
{
	if(trg)
	{
		...
		if (trg->isObjType(OBJTYPE_Vehicle))
		{
			...
			if (vehicle->GetVehicleType() == obj_Vehicle::VEHICLETYPE_COPCAR)
				attackRadius *= IsDogZombie()?3.5f:2.5f;
...
BOOL obj_Zombie::Update()
{
	...
	if(ZombieState == EZombieStates::ZState_Dead)
	{
		...
		if(curTime > StateStartTime + 10.0f)
			setActiveFlag(0);
		return TRUE;
	}
	...
	switch(ZombieState)
	{
		...
		case EZombieStates::ZState_Attack:
		{
			...
			if (gameObj->Class->Name == "obj_Door")
			{
				...
				if (Door->m_Health >= 0)
				{
					if (Door->m_OpenDoor == 1) // 1 = door open, 0 = door close
					{
						StopAttack();
						break;
					}
...
void obj_Zombie::DoDeath(bool fakeDeath)
{
    ...
    {
        int HowManyToDrop = 1;
        if( IsSuperZombie()) HowManyToDrop = 12; // SuperZombie Drop Amount
	...
	if(!fakeDeath && spawnObject->lootBoxCfg)
	{
			for(int i=0; i<2; i++) // 1 is Item drop per 1 zombie
			{
	...
	{
		for(int i=0; i<1; i++) // 10 is item drop per 1 serperzombie // By NoverZ
		{
			wiInventoryItem wi = RollItem(spawnObject->lootBoxCfg, 0);
			if(wi.itemID > 0)
			{
