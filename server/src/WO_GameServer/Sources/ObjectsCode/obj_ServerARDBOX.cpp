...
#include "obj_ServerARDBOX.h"
#include "ServerGameLogic.h"
#include "../EclipseStudio/Sources/ObjectsCode/weapons/WeaponArmory.h"
#include "../../GameEngine/ai/AutodeskNav/AutodeskNavMesh.h"
#include "Async_ServerObjects.h"

IMPLEMENT_CLASS(obj_ServerARDBOX, "obj_ServerARDBOX", "Object");
AUTOREGISTER_CLASS(obj_ServerARDBOX);

const static int ARDBOX_NOT_USED_EXPIRE_TIME = 1 * 60;
const static int ARDBOX_LOCKDOWN_TIME        = 5;

obj_ServerARDBOX::obj_ServerARDBOX()
{
	float expireTime = r3dGetTime() + ARDBOX_NOT_USED_EXPIRE_TIME;
	...
	//srvObjParams_.ExpireTime = r3dGetTime() + ARDBOX_NOT_USED_EXPIRE_TIME;
...
BOOL obj_ServerARDBOX::OnCreate()
{
	...
	//ARDBOXOwnerId = srvObjParams_.CustomerID;

	// convert old ARDBOXs with unlimited expire time to current expiration time
	if((int)(srvObjParams_.ExpireTime - r3dGetTime()) > ARDBOX_NOT_USED_EXPIRE_TIME)
	{
		UpdateServerData();
	}
...
BOOL obj_ServerARDBOX::OnDestroy()
{
	if(m_ObstacleId >= 0)
	{
...
wiInventoryItem* obj_ServerARDBOX::FindItemWithInvID(__int64 invID)
{
...
bool obj_ServerARDBOX::AddItemToARDBOX(const wiInventoryItem& itm, int quantity)
{
	for(size_t i=0; i<items.size(); ++i)
	{
...
void obj_ServerARDBOX::RemoveItemFromARDBOX(__int64 invID, int amount)
{
	r3d_assert(amount >= 0);
	...
	for(size_t i=0; i<items.size(); ++i)
	{
		if(items[i].InventoryID == invID)
		{
			...
			// remove from ARDBOX items array
...
BOOL obj_ServerARDBOX::Update()
{
	...
	{
		m_nextLockdownClear = curTime + 1.0f;
...
void obj_ServerARDBOX::setAccessCode(const char* newCodeS)
{
...
void obj_ServerARDBOX::SendContentToPlayer(obj_ServerPlayer* plr)
{
	PKT_S2C_ARDBOXOpReq_s n2;
	n2.op = PKT_S2C_ARDBOXOpReq_s::LBOR_StartingToSendContent;
	n2.ARDBOXID = toP2pNetId(GetNetworkID());
	gServerLogic.p2pSendToPeer(plr->peerId_, this, &n2, sizeof(n2));	

	PKT_S2C_ARDBOXContent_s n;
	...
	n2.op = PKT_S2C_ARDBOXOpReq_s::LBOR_DoneSendingContent;
	n2.ARDBOXID = toP2pNetId(GetNetworkID());
	gServerLogic.p2pSendToPeer(plr->peerId_, this, &n2, sizeof(n2));	
}

bool obj_ServerARDBOX::IsLockdownActive(const obj_ServerPlayer* plr)
{
	const float curTime = r3dGetTime();
	...
	for(size_t i=0; i<m_lockdowns.size(); i++)
	{
		...
		if(lock.CustomerID == plr->profile_.CustomerID && curTime < lock.lockEndTime)
		{
			...
			if(lock.tries > ARDBOX_LOCKDOWN_TIME)
			{
				gServerLogic.LogCheat(plr->peerId_, PKT_S2C_CheatWarning_s::CHEAT_ARDBOX, false, "ARDBOX",
					"tries %d", lock.tries);
			}
			return true;
		}
...
void obj_ServerARDBOX::SetLockdown(DWORD CustomerID)
{
	float lockEndTime = r3dGetTime() + ARDBOX_LOCKDOWN_TIME;
...
bool obj_ServerARDBOX::IsARDBOXAbused(const obj_ServerPlayer* plr)
{
	const float curTime = r3dGetTime();
...
void obj_ServerARDBOX::DestroyARDBOX()
{
	setActiveFlag(0);
	g_AsyncApiMgr->AddJob(new CJobDeleteServerObject(this));
}

void obj_ServerARDBOX::UpdateServerData()
{
	// if lockbox was used, extend expire time for 2 weeks
	srvObjParams_.ExpireTime = srvObjParams_.CreateTime + ARDBOX_NOT_USED_EXPIRE_TIME;
	g_AsyncApiMgr->AddJob(new CJobUpdateServerObject(this));
}

DefaultPacket* obj_ServerARDBOX::NetGetCreatePacket(int* out_size)
{
...
void obj_ServerARDBOX::LoadServerObjectData()
{
...
void obj_ServerARDBOX::SaveServerObjectData()
{
