...
class obj_ServerARDBOX: public GameObject, INetworkHelper
{
	DECLARE_CLASS(obj_ServerARDBOX, GameObject)
public:
	uint32_t	m_ItemID; // itemID of ARDBOX
	int		m_ObstacleId;
	float		m_Radius;
	DWORD		ARDBOXOwnerId;
	...
	std::vector<wiInventoryItem> items; // all items in this ARDBOX
	uint32_t	maxItems; // max items this ARDBOX can hold
	...
	std::vector<lock_s> m_uses;		// used for tracking usage-per-seconds for lockboxes, lockEndTime used as ARDBOX opening time
...
public:
	...
	bool		IsARDBOXAbused(const obj_ServerPlayer* plr);
	void		DestroyARDBOX();
	void		UpdateServerData();

	wiInventoryItem* FindItemWithInvID(__int64 invID);
	bool		AddItemToARDBOX(const wiInventoryItem& itm, int quantity);
	void		RemoveItemFromARDBOX(__int64 invID, int amount);
