...
class obj_ServerDropped;
class obj_ServerARDBOX;
...
class obj_ServerPlayer : public GameObject, INetworkHelper
{
  public:
	...
	float		 startColdTime_;
	float        startDmgTime_;
	float        startRadTime_;
	
	bool        justTeleported;
	bool        PlayerIsOnBattle;
	bool        playersTeleportedToStartPosition;
	...
	//bool		inWounded; // wounded disabled by now
	//int			resurrected; // wounded disabled by now
	//bool		trueDeath; // wounded disabled by now
	//float		inWoundedTimer; // wounded disabled by now
	//GameObject* sourceObjForinWounded; // wounded disabled by now
	//obj_ServerPlayer* targetPlrForinWounded; // wounded disabled by now
	//STORE_CATEGORIES weaponCatForinWounded; // wounded disabled by now
	//bool		forced_by_serverForinWounded; // wounded disabled by now
	//bool		fromPlayerInAirForWounded;  // wounded disabled by now
	//bool		targetPlayerInAirForinWounded; // wounded disabled by now
	void		DoKillPlayer(GameObject* sourceObj, obj_ServerPlayer* targetPlr, STORE_CATEGORIES weaponCat, bool forced_by_server, bool fromPlayerInAir, bool targetPlayerInAir );
 ...
 //
	...
	bool		isBattleRoyale; //battleroyale
	...
	void		SetLasSknUsed(int itemID, int SkinID);
	bool		SetItemSkinDB(int itemID, int SkinID);
	bool		CheckSkinGear(int itemID, int SkinID);
	int SkinsCount;
	...
	obj_ServerDropped* GetAccessToDropped(gp2pnetid_t droppedID, const char* AccessCodeS);
	obj_ServerARDBOX* GetAccessToARDBOX(gp2pnetid_t ARDBOXID, const char* AccessCodeS);
	...
	void		TeleportPlayerToBattleRoyaleLobby(obj_ServerPlayer* plr); // WORK IN PROGRESS
	void		TeleportPlayerToBattleArena(obj_ServerPlayer* plr); // WORK IN PROGRESS
	...
	bool		CaseAddItem(const PKT_C2S_OpenCase_s& n2, const wiInventoryItem& wi1);
	...
	bool		UseItem_Dropped(const r3dPoint3D& pos, float rotX, uint32_t itemID);
	bool		UseItem_ARDBOX(const r3dPoint3D& pos, float rotX, uint32_t itemID);
...
public:
	...
	void		OnNetPacket(const PKT_C2C_StartHorse_s& n);
	...
	void		OnNetPacket(const PKT_C2C_StartDance_s& n);
	void		OnNetPacket(const PKT_C2C_StartBackflip_s& n);
	
	void		OnNetPacket(const PKT_C2C_StartSitdown_s& n);
	void		OnNetPacket(const PKT_C2C_StartSwinslow_s& n);
	void		OnNetPacket(const PKT_C2C_StartPlayerdead_s& n);
	void		OnNetPacket(const PKT_C2C_StartCrounched_s& n);

	void		OnNetPacket(const PKT_C2C_StartPickup_s& n);
	void		OnNetPacket(const PKT_C2C_StartDrink_s& n);
	void		OnNetPacket(const PKT_C2C_StartTalk_s& n);
	void		OnNetPacket(const PKT_C2C_StartRepair_s& n);
	void		OnNetPacket(const PKT_C2C_StartEat_s& n);
	...
	void		OnNetPacket(const PKT_C2S_OpenCase_s& n);
	...
	void        OnNetPacket(const PKT_C2S_PlayerDropArrow_s& n);
	void		OnNetPacket(const PKT_C2S_PlayerChangeBackpack_s& n);
	void		OnNetPacket(const PKT_C2S_BackpackDrop_s& n);
	void		OnNetPacket(const PKT_C2S_BackpackDropAll_s& n); //Drop all items
	...
	void		OnNetPacket(const PKT_C2S_QuickRevive_s& n);
	...
	void		OnNetPacket(const PKT_C2S_DroppedItemDroppedToBackpack_s& n);
	void		OnNetPacket(const PKT_C2S_DroppedItemBackpackToDropped_s& n);
	void		OnNetPacket(const PKT_C2S_DroppedPickup_s& n);
	void		OnNetPacket(const PKT_C2S_ARDBOXItemARDBOXToBackpack_s& n);
	void		OnNetPacket(const PKT_C2S_ARDBOXItemBackpackToARDBOX_s& n);
	void		OnNetPacket(const PKT_C2S_ARDBOXPickup_s& n);
...
/////////////////////// Character Voice Command ////////////////////
	void		OnNetPacket(const PKT_C2C_CharVoiceOne_s& n);
	void		OnNetPacket(const PKT_C2C_CharVoiceTwo_s& n);
	void		OnNetPacket(const PKT_C2C_CharVoiceThree_s& n);
	void		OnNetPacket(const PKT_C2C_CharVoiceFour_s& n);
	void		OnNetPacket(const PKT_C2C_CharVoiceFive_s& n);
	void		OnNetPacket(const PKT_C2C_CharVoiceSix_s& n);
	void		OnNetPacket(const PKT_C2C_CharVoiceSeven_s& n);
	void		OnNetPacket(const PKT_C2C_CharVoiceEight_s& n);
	void		OnNetPacket(const PKT_C2C_CharVoiceNine_s& n);
/////////////////////// Character Voice Command ////////////////////
	...
	void	   OnNetPacket(const PKT_S2C_UpdateWpnSkinSelected_s& n);
	void		OnNetPacket(const PKT_C2S_LearnSkinItem_s& n);
