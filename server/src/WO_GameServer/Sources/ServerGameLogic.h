...
public:
  ...
  public:
	...
	float		AirBombSpawnTime;
	float		isDroppedTimeExpired;
	...
	float m_StartGameTime; //battleroyale
	float m_StartGameTimeR; //battleroyale
	int countTimeFinish; //battleroyale
	bool		ForceStarGame;//battleroyale
	...
	void		ResetPositionBattleRoyale(obj_ServerPlayer* plr, int lbNum);
	void		GetStartPositionBattleRoyale2(obj_ServerPlayer* plr);
	void		DestroyBarricadesBattleRoyale();
	void		ReSpawnLootBattleRoyale();
	...
	obj_ServerDropped* TryToOpenDropped(obj_ServerPlayer* fromPlr, gp2pnetid_t droppedID, const char* AccessCodeS);
	obj_ServerARDBOX* TryToOpenARDBOX(obj_ServerPlayer* fromPlr, gp2pnetid_t ARDBOXID, const char* AccessCodeS);
	...
	int		ProcessWorldEvent(GameObject* fromObj, DWORD eventId, DWORD peerId, const void* packetData, int packetSize);
	 ...
	 DEFINE_PACKET_FUNC(PKT_C2S_DroppedOpAns);
	 DEFINE_PACKET_FUNC(PKT_C2S_DroppedKeyReset);
	 DEFINE_PACKET_FUNC(PKT_C2S_ARDBOXOpAns);
	 DEFINE_PACKET_FUNC(PKT_C2S_ARDBOXKeyReset);
	...
	int       Cmd_ReportSS(obj_ServerPlayer* plr, const char* cmd); // System send ScreenShot
	...
	int		  Cmd_AirBomb(obj_ServerPlayer* plr, const char* cmd);
...
#endif
	bool		ApplyDamageToDoor(GameObject* fromObj, GameObject* targetDoor, const r3dPoint3D& dmgPos, float damage, bool forceDamage, STORE_CATEGORIES damageSource, uint32_t damageItemId);
	...
	void		InformZombiesAboutChaosExplosion(const obj_ChaosObject* chaos);
#ifdef VEHICLES_ENABLED
	...
	void		ApiPlayerUpdateWeaponSkins(obj_ServerPlayer* plr);
	...
	std::map<uint32_t, AirBombPositions> AirBombsPos;
	...
	bool		SetAirBomb( const AirBombPositions& AirBomb );
  ...
  public:
	...
	void		GetStartPositionBattleRoyale(obj_ServerPlayer* plr, bool& needsSpawnProtection);
	float		safezoneTime;
	bool		playersTeleported;
	float		teleportTimer;
	float		canCheckPlayers;
	void		TeleportPlayerToBattleRoyaleLobby(obj_ServerPlayer* plr);
	bool		isBRMatchEnd;
	void		BattleRoyaleMatchEnd();
