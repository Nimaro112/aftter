...
void gameServerLoop()
{
  ...
  _mkdir("Logs_GameServer");
  _mkdir("Reports");
  char fname[MAX_PATH];
  sprintf(fname, "Logs_GameServer\\GS_%I64x.txt", cfg_sessionId);
  r3dChangeLogFile(fname);
  
  sprintf(fname, "Logs_GameServer\\GS_%I64x.dmp", cfg_sessionId);
  SrvSetCrashHandler(fname);

  sprintf(fname, "AfterLife MMO GameServer, port %d", cfg_hostPort);
  ...
  gMasterServerLogic.Connect("25.91.238.22", SBNET_SUPER_GAME_PORT, cfg_hostPort + SBNET_LISTEN_PORT_ADD);
