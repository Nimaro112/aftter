...
int ServerWeapon::getClipSize() const
{
	//uint32_t clipsizeBoost = 0;
	//clipsizeBoost += (int)m_WeaponAttachmentStats[WPN_ATTM_STAT_CLIPSIZE];

	return 0 /*m_pConfig->m_clipSize+clipsizeBoost*/;
}

void ServerWeapon::Reset()
{
	//m_NumClips   = m_pConfig->m_Clipsize;
...
void ServerWeapon::Resupply()
{
	// total bullets left to shoot
	m_NumBulletsLeft = m_Clipsize * getClipSize();
}
