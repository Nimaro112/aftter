...
int CTeamSpeakServer::Startup()
{
	...
	serverID = CreateVirtualServer("AFTERLIFE", m_serverPort, PROVISIONING_SLOTS_BUFFER);
	if(!serverID)
		return 0;

	// Set welcome message
	ts3server_setVirtualServerVariableAsString(serverID, VIRTUALSERVER_WELCOMEMESSAGE, "AFTERLIFE");
...
uint64 CTeamSpeakServer::CreateVirtualServer(const char* name, int port, int maxClients)
{
	...
	if((error = ts3server_createVirtualServer(port, "25.91.238.22", name, keyPair, maxClients, &serverID)) != ERROR_ok)
