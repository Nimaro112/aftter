...
void InitObjCategories()
{
	...
	while (!catEntry.empty())
	{
		...
		{
			WIN32_FIND_DATA ffblk;
			HANDLE h = FindFirstFile("data\\particles\\*.pt2", &ffblk);
			if(h != INVALID_HANDLE_VALUE) do 
			{
				// strip .pt2
			...
			if (imgui_Button(SliderX + 125, SliderY, 115, 25, "Export NoClip"))
			{
				...
				if( obj_NoClipBox::s_vNoClipData.size() > 0 || obj_NoClipPlane::noClipDatas.size() > 0)
				{
					...
					if( mapname )
					{
						...
						if(stricmp(mapname+1, "alpha_island")==0)
							mapId = GBGameInfo::MAPID_AM_Island;
						else if(stricmp(mapname+1, "am_battleroyale")==0)
							mapId = GBGameInfo::MAPID_AM_Battleroyale;
						else if(stricmp(mapname+1, "am_compound")==0)
							mapId = GBGameInfo::MAPID_AM_Compound;
						else if(stricmp(mapname+1, "am_convention_center")==0)
							mapId = GBGameInfo::MAPID_AM_ConventionC;
						else if(stricmp(mapname+1, "am_devmap")==0)
							mapId = GBGameInfo::MAPID_AM_Devmap;
						else if(stricmp(mapname+1, "am_pinetop")==0)
							mapId = GBGameInfo::MAPID_AM_PineTop;
						else if(stricmp(mapname+1, "am_RotB_lms_01")==0)
							mapId = GBGameInfo::MAPID_AM_RoTB;
						else if(stricmp(mapname+1, "am_nevada")==0)
							mapId = GBGameInfo::MAPID_AM_Nevada;
						else if(stricmp(mapname+1, "am_southwest")==0)
							mapId = GBGameInfo::MAPID_AM_SouthWest;
						else if(stricmp(mapname+1, "am_compound")==0)
							mapId = GBGameInfo::MAPID_AM_RentServer01;
						else if(stricmp(mapname+1, "am_convention_center")==0)
							mapId = GBGameInfo::MAPID_AM_RentServer02;
					}
					else
					{
		...
		case PE_RAIN:
			{
				...
				if( InSelectParticleMode )
				{
					...
					if (imgui_FileList(5, 80, 300, 380, "Data/Particles/*.pt2", ParticlePath, &offset, false, valueChanged ))
					{
	...
	if ( bShowPreview || bDragPreview )
	{
		...
		else if ( nIndex == 4 )
		{
			...
			if ( bNewSelect )
			{
				...
				sprintf(buffer,"Data\\Particles\\%s.pt2", fileName);
...
void SpawnZombie()
{
	...
	czs.HairIdx			= rand() % heroConfig->getNumHairs(); // hair system - not for zombies
