...
bool storecat_IsItemStackable(uint32_t ItemID)
{
	...
	// NOTE: MAKE SURE to sync this with am_gamedb.dbo.FN_IsItemStackable
	switch(itm->category)
	{
...
int CUserProfile::GetProfile(int CharID)
{
	...
	ParseWeaponSkins(xmlAccount.child("weaponskins"));
...
void CUserProfile::ParseLoadouts(pugi::xml_node& xmlItem)
{
	...
	while(!xmlItem.empty())
	{
		...
		w.HairIdx     = xmlItem.attribute("HairIdx").as_int(); // hair system
		...
		st.VictorysHardGames  = xmlItem.attribute("ts03").as_int();//battleroyale
...
void CUserProfile::ParseWeaponSkins(pugi::xml_node& xmlItem)
{
	...
	while(!xmlItem.empty())
	{
		...
		if (WpnItemsID[LastWpn] != itemID)
		{
			LastWpn=WpnCount;
			WpnItemsID[WpnCount] = itemID;
			SelectedSKN[WpnCount] = Selected;
			WpnCount++;
			SknCount[LastWpn]=0;
		}

		WpnSkins[LastWpn][SknCount[LastWpn]] = Skin;
		SknCount[LastWpn]++;
		xmlItem = xmlItem.next_sibling();
	}
		for (int i=0;i<WpnCount;i++)
		{
			...
			w.itemID = WpnItemsID[i];
			w.LastSKINused = SelectedSKN[i];
			for (int a=0;a<SknCount[i];a++)
			{				
				w.Skins[a] = WpnSkins[i][a];
			}
		}
}
...
int CClientUserProfile::ApiCharCreate(const char* Gamertag, int Hardcore, int HeroItemID, int HairIdx, int HeadIdx, int BodyIdx, int LegsIdx)
{
	...
	req.AddParam("HairIdx",    HairIdx); // hair system
...
int CClientUserProfile::ApiCharChangeSkin(int HairIdx, int HeadIdx, int BodyIdx, int LegsIdx, int HeroItemID)
{
...
int CClientUserProfile::ApiGetLeaderboard(int TableID, int StartPos, int* out_CurPos, int* lbSize)
{
	...
	*lbSize = xmlLeaderboard.attribute("size").as_int(); 
	...
	while(!xmlItem.empty())
	{
		...
		lb.stats.VictorysHardGames   = xmlItem.attribute("ts03").as_uint();//battleroyale
