...
enum STORE_CATEGORIES
{
	...
	storecat_Mask			= 9, 	// new mask for head gear
	storecat_Scarf			= 10,   // new scarf for head gear
	storecat_Armor			= 11,
	...
	storecat_CraftRecipe	= 51,	// Blueprints cards
	storecat_Vehicle		= 55,   // vehicle
	storecat_Skins 			= 56,   // Skins
	storecat_Cases			= 57,   // case ing system
...
static const uint32_t MAX_SKINS_BY_WEAPON = 30;
static const uint32_t MAX_WEAPONS_SKINEABLED = 999;
...
struct wiStats
{
	...
	int		KilledSurvivors;
	int		KilledBandits;
	int		VictorysHardGames;//battleroyale
...
// NOTE: you MUST increase P2PNET_VERSION if you change this structure
	...
	enum
	{
	  GAMEFLAG_NearPostBox 		= (1 << 0), // Safe Zone Area
	  GAMEFLAG_NearDmgBox	    = (1 << 1), // Damage Area
	  GAMEFLAG_NearRadBox 		= (1 << 2), // RadioActive Area
	  GAMEFLAG_NearColdBox 		= (1 << 3), // Cold Damage Area
	  GAMEFLAG_NearWaterWell 	= (1 << 4), // WaterWell box
	  GAMEFLAG_NearCookingRack  = (1 << 5), // Cooking box
	  GAMEFLAG_NearGasProp 		= (1 << 6), // gasprop box
	  GAMEFLAG_NearStairs		= (1 << 7), // gasprop box
	  GAMEFLAG_NearBattleArena	= (1 << 8) // Battle Arena
	};
	...
	enum 
	{
	  ...
	  CHAR_LOADOUT_MASKGEAR,
	  CHAR_LOADOUT_SCARFGEAR,
	...
	// learned recipes
	static const int MAX_NUM_RECIPES_IN_LOADOUT=128; // max recipe learned vault goes here
...
struct wiUserProfile
{
	...
	int  HardMaxPlayers;//battleroyale
...
class CUserProfile
{
  ...
  public:
	...
	void		 ParseWeaponSkins(pugi::xml_node& xmlItem);
...
class CClientUserProfile : public CUserProfile
{
  ...
  public:
	...
	int		ApiCharCreate(const char* Gamertag, int Hardcore, int HeroItemID, int HairIdx, int HeadIdx, int BodyIdx, int LegsIdx);
	int		ApiCharDelete();
	int		ApiCharRevive(int NeedMoney);
	int		ApiCharReviveCheck(int* out_needMoney, int* out_secToRevive);
	int		ApiCharChangeSkin(int HairIdx, int HeadIdx, int BodyIdx, int LegsIdx, int HeroItemID); // change skin of current survivor
