...
#ifdef FINAL_BUILD
const char * g_szApplicationName = "AfterLife MMO"; // Client
#else
const char * g_szApplicationName = "AfterLife MMO"; // Studios
#endif 

int32_t	g_nProjectVersionMajor = 1; // version
int32_t	g_nProjectVersionMinor = 5; // version
int32_t g_nProjectVersionMinor2= 0; // version
...
void game::PreInit()
{
...
#ifdef FINAL_BUILD
	{
		...
		{
			MessageBox(NULL, "Please run the AfterLifeMMO launcher next time. Press OK to open the launcher.", g_szApplicationName, MB_OK);
			ShellExecute(NULL, "open", "Game_Launcher.exe", NULL, NULL, SW_SHOWDEFAULT);
			ExitProcess(0);
		}
		...
		{
			MessageBox(NULL, "Please run Game launcher without Sandbox.", g_szApplicationName, MB_OK);
			ExitProcess(0);
		}
	}
#endif	
...
//		g_bEditMode = false;
//		UpdateDB("107.173.207.141", "Data/Weapons/SQL_itemsDB.xml");
//		MessageBox(0, "Successfully updated DB!", "Result", MB_OK);
//		break;
//#endif
	...
	case Menu_AppSelect::bStartGamePublic:
		...
		if(strstr(__r3dCmdLine, "-ffgrtvzdf") == NULL)
		{
			...
			g_serverip->SetString("107.173.207.141");
		}

		// override API settings
		g_api_ip->SetString("107.173.207.141");
...
void UpdateDB(const char* api_addr, const char* out_xml)
{
	...
	// get items DB
	{
		CkHttpRequest req;
		req.UsePost();
		req.put_Path("/GameAPI/app/php/api_getItemsDB.php");
...
void ExecuteCharacterEditor()
{
	PrepareEditor("ONLINE EDITOR/Editor_Base");
	CurHUDID = 5;
	PlayEditor();
}
...
void ExecuteParticleEditor()
{
	PrepareEditor("ONLINE EDITOR/Editor_Particles");
	CurHUDID = 3;
	PlayEditor();
}
