...
#include "../../../External/PunkBuster/pbcommon.h"
	...
	char		_p2p_masterHost[MAX_PATH] = "107.173.207.141";
	int		_p2p_masterPort = GBNET_CLIENT_PORT;
static	char		_p2p_gameHost[MAX_PATH] = "107.173.207.141";
...
void loadFrontend()
{
	...
	//if(gUserProfile.ProfileData.isDevAccount)
		//frontend = game_new FrontendWarZ("Data\\Interface\\Dev_Lobby.swf");
	//else
		frontend = game_new FrontendWarZ("Data\\Interface\\Game_FrontEnd.swf");
...
void ExecuteNetworkGame()
{
	...
	// bink video
	{
		CutScene* startMovie = game_new CutScene;
		startMovie->Initialize(win::hWnd, "Data\\video\\startmovie.bik");
		for (;;)
		{
			if (!startMovie->playMovie())
			{
			}
			else
				break;
	...
	int mainmenuTheme = -1;
	mainmenuTheme = SoundSys.GetEventIDByPath("Sounds/AM_Group/MainMenu/ThemeMenu-01");
