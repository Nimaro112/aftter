...
int Menu_AppSelect::DoModal()
{
	...
	CTestMenu* menu = new (ALLOC_TYPE_LOGIC) CTestMenu("data/Interface/HUD_OutOfBattleZone.swf"); //HUD_OutOfBattleZone
	menu->Load();
	int res = 0;
	if(menu->Initialize())
	{
	...
	while(1)
	{
		...
		case 100:
			{
				const static char *BNames1[] = {"Play Game" };
				...
				for (int i=0;i<R3D_ARRAYSIZE(BNames1);i++)
					if (imgui_Button(r3dRenderer->ScreenW/2-(210*R3D_ARRAYSIZE(BNames1))/2+210*i, r3dRenderer->ScreenH/2 - 30,200, 30,BNames1[i], 0)) 
						released_id = bStartGameSVN+i;

				const static char* BNames[] = {"Map Editor", "Particles Editor", "Physx Editor", "Character Editor" };
