...
int Menu_Main::DoModal()
{
 ...
 char tempName[256] = {0};
  ...
  while(1)
  {
	...
	const static char* list[3] = { "LIVE MAPS", "ONLINE MAPS", "CREATE MAP"};
	...
	switch (LevelIndex)
	{
			...
			case	1: // EDITOR MAPS
				{
					static float offset = 0.f ;
					imgui_FileList(r3dRenderer->ScreenW/2-240, r3dRenderer->ScreenH/2-140+2, 360, 330, "Levels\\ONLINE EDITOR\\*.", tempName, &offset );

					if (tempName[0])
					{
						sprintf(LevelEditName, "ONLINE EDITOR\\%s", tempName);
						if (imgui_Button(r3dRenderer->ScreenW/2+250-150, r3dRenderer->ScreenH/2+200+5,150,30, "Load Level", 0))
							released_id = bEditor;
					}
				}
