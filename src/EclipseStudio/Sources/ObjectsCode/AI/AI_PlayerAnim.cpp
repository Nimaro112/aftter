...
void CUberData::LoadPlayerTauntingAnim()
{
	aid_.playertaunt[0] = AddAnimation("Taunt_t1_Litterbox");
	aid_.playertaunt[1] = AddAnimation("Taunt_t1_Spank");
	aid_.playertaunt[3] = AddAnimation("Taunt_t1_HandFlip");
	aid_.playertaunt[4] = AddAnimation("Taunt_t1_Jack");
	aid_.playertaunt[5] = AddAnimation("Taunt_t1_PThrust");
	aid_.playertaunt[6] = AddAnimation("Taunt_t1_dance");
	aid_.playertaunt[7] = AddAnimation("Taunt_t1_backflip");
	
	aid_.playertaunt[8] = AddAnimation("UI_Sit_01");
	aid_.playertaunt[9] = AddAnimation("Swim_Slow_Forward");
	aid_.playertaunt[10] = AddAnimation("Zombie_Stand_Dead_B_01");
	aid_.playertaunt[11] = AddAnimation("Zombie_Eat_Crouched_Idle_01");
}

void CUberData::LoadPlayerInteractAnim()
{
	aid_.PlayerInteract[0] = AddAnimation("PickUpItem");
	aid_.PlayerInteract[1] = AddAnimation("Drink_Anim");
	aid_.PlayerInteract[2] = AddAnimation("PlayerTalkInteract");
	aid_.PlayerInteract[3] = AddAnimation("RepairBenchAnim");
	aid_.PlayerInteract[4] = AddAnimation("PlayerEatCan");
}
...
void CUberData::LoadAnimations()
{
	...
	aid_.InspectWeapon[0] = AddAnimation("FPS_Weap_Show"); //ASR
	aid_.InspectWeapon[1] = AddAnimation("FPS_Weap_Show"); //SNP
	aid_.InspectWeapon[2] = AddAnimation("FPS_Weap_Show"); //SHG
	aid_.InspectWeapon[3] = AddAnimation("FPS_Weap_Show"); //MG
	aid_.InspectWeapon[4] = AddAnimation("FPS_Weap_Show"); //HG
	aid_.InspectWeapon[5] = AddAnimation("FPS_Weap_Show"); //SMG
...
{
	const char* skel_fname = "data/ObjectsDepot/CharactersNew/ProperScale_AndBiped.skl";
...
void CUberEquip::DrawSlot(ESlot slotId, const D3DXMATRIX& world, DrawType dt, bool skin, bool draw_firstperson, const r3dSkeleton* wpnSkeleton)
{
	...
	if(draw_firstperson)
	{
		if(slotId == SLOT_Armor || slotId == SLOT_LowerBody || slotId == SLOT_Hair || slotId == SLOT_Head || slotId == SLOT_Helmet || slotId == SLOT_Scarf || slotId == SLOT_Mask || slotId == SLOT_Backpack)
			return;
	}
	...
	if(slots_[slotId].gear)
		...
		mesh = slots_[slotId].wpn->getMeshSkin(isFirstPerson&&draw_firstperson);
	else if(slots_[slotId].mesh)
...
CUberAnim::CUberAnim(obj_Player* in_player, CUberData* in_data)
{
	...
	jumpAnimSpeed           = 2.1f; // 1.9
	jumpStartTime           = 0.1f;	// 0.3
	jumpStartTimeByState[0] = 0.1f; // idle 0.3
...
void CUberAnim::SyncAnimation(int PlayerState, int MoveDir, bool force, const Weapon* weap, bool isInAttmMenu, bool isWantingToInspect)
{
	if(weap)
	{
		...
		if(PlayerState == PLAYER_IDLE && ((r3dGetTime() < weap->getLastTimeFired() + 60.0f) && !IsFPSMode())) // 5sec delay before returning back to idle
			PlayerState = PLAYER_IDLEAIM;

		// switch state if we firing in idle mode
		if(PlayerState == PLAYER_PRONE_IDLE && ((r3dGetTime() < weap->getLastTimeFired() + 60.0f) && !IsFPSMode())) // 5sec delay before returning back to idle
...
void CUberAnim::StartHorseAnim()
{
	StopReloadAnim();	
	anim.StartAnimation(data_->aid_.playertaunt[0], ANIMFLAG_Looped | ANIMFLAG_RemoveOtherFade, 0.0f, 1.0f, 0.1f);
}
...
void CUberAnim::StartSpankAnim()
{
	...
	StopReloadAnim();
	anim.StartAnimation(data_->aid_.playertaunt[1], ANIMFLAG_Looped | ANIMFLAG_RemoveOtherFade, 0.0f, 1.0f, 0.1f);
}

void CUberAnim::StartHandFlipAnim()
{
	StopReloadAnim();
	anim.StartAnimation(data_->aid_.playertaunt[3], ANIMFLAG_Looped | ANIMFLAG_RemoveOtherFade, 0.0f, 1.0f, 0.1f);
}

void CUberAnim::StartJackAnim()
{
	StopReloadAnim();
	anim.StartAnimation(data_->aid_.playertaunt[4], ANIMFLAG_Looped | ANIMFLAG_RemoveOtherFade, 0.0f, 1.0f, 0.1f);
}

void CUberAnim::StartPThrustAnim()
{
	StopReloadAnim();
	anim.StartAnimation(data_->aid_.playertaunt[5], ANIMFLAG_Looped | ANIMFLAG_RemoveOtherFade, 0.0f, 1.0f, 0.1f);
}

void CUberAnim::StartDanceAnim()
{
	StopReloadAnim();	
	anim.StartAnimation(data_->aid_.playertaunt[6], ANIMFLAG_Looped | ANIMFLAG_RemoveOtherFade, 0.0f, 1.0f, 0.1f);
}

void CUberAnim::StartBackflipAnim()
{
	StopReloadAnim();	
	anim.StartAnimation(data_->aid_.playertaunt[7], ANIMFLAG_Looped | ANIMFLAG_RemoveOtherFade, 0.0f, 1.0f, 0.1f);
}
/////
void CUberAnim::StartSitdownAnim()
{
	StopReloadAnim();	
	anim.StartAnimation(data_->aid_.playertaunt[8], ANIMFLAG_Looped | ANIMFLAG_RemoveOtherFade, 0.0f, 1.0f, 0.1f);
}
void CUberAnim::StartSwinslowAnim()
{
	StopReloadAnim();	
	anim.StartAnimation(data_->aid_.playertaunt[9], ANIMFLAG_Looped | ANIMFLAG_RemoveOtherFade, 0.0f, 1.0f, 0.1f);
}
void CUberAnim::StartPlayerdeadAnim()
{
	StopReloadAnim();	
	anim.StartAnimation(data_->aid_.playertaunt[10], ANIMFLAG_Looped | ANIMFLAG_RemoveOtherFade, 0.0f, 1.0f, 0.1f);
}
void CUberAnim::StartCrounchedAnim()
{
	StopReloadAnim();	
	anim.StartAnimation(data_->aid_.playertaunt[11], ANIMFLAG_Looped | ANIMFLAG_RemoveOtherFade, 0.0f, 1.0f, 0.1f);
}
/////
///////////////////////////////////////////////////////////// PLAYER INTERACT /////////////////////////////////////////////////////////////
...
void CUberAnim::StartDrinkAnim()
{
	StopReloadAnim();	
	anim.StartAnimation(data_->aid_.PlayerInteract[1], ANIMFLAG_PauseOnEnd | ANIMFLAG_RemoveOtherFade, 0.0f, 1.0f, 0.1f);
}

void CUberAnim::StartTalkAnim()
{
	StopReloadAnim();	
	anim.StartAnimation(data_->aid_.PlayerInteract[2], ANIMFLAG_PauseOnEnd | ANIMFLAG_RemoveOtherFade, 0.0f, 1.0f, 0.1f);
}

void CUberAnim::StartRepairAnim()
{
	StopReloadAnim();	
	anim.StartAnimation(data_->aid_.PlayerInteract[3], ANIMFLAG_PauseOnEnd | ANIMFLAG_RemoveOtherFade, 0.0f, 1.0f, 0.1f);
}

void CUberAnim::StartEatAnim()
{
	StopReloadAnim();	
	anim.StartAnimation(data_->aid_.PlayerInteract[4], ANIMFLAG_PauseOnEnd | ANIMFLAG_RemoveOtherFade, 0.0f, 1.0f, 0.1f);
}
