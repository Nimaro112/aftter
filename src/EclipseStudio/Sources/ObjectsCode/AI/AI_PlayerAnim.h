...
static	int		GetMoveDirFromAcceleration(const r3dPoint3D& accel);
	...
	struct animIndices_s
	{
	  ...
	  int		jumps[20];
	  int		deaths[20];
	  int		playertaunt[20]; // for player taunt system
	  int		PlayerInteract[20]; // for player Interactive with objs
	  int		playerinteract[20];
...
class CUberAnim
{
  ...
  public:
	...
	void		StartDanceAnim();
	void		StartBackflipAnim();
	void		StartSitdownAnim();
	void		StartSwinslowAnim();
	void		StartPlayerdeadAnim();
	void		StartCrounchedAnim();
