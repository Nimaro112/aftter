...
obj_AnimalDummy::obj_AnimalDummy()
{
	...
	if(g_animalBindSkeleton == NULL) 
	{
		...
		g_animalBindSkeleton->LoadBinary("Data\\ObjectsDepot\\G3_Animals\\Zombie_Dog.skl");
...
BOOL obj_AnimalDummy::OnCreate()
{
	...
	physSkeleton = game_new r3dPhysDogSkeleton( "data/ObjectsDepot/G3_Animals/ZDog.RepX" ) ;
...
{
	char buf[MAX_PATH];
	sprintf(buf, "Data\\ObjectsDepot\\G3_Animals\\Animations\\%s.anm", anim);
...
{
	char buf[MAX_PATH];
	sprintf(buf, "Data\\ObjectsDepot\\G3_Animals\\Animations\\%s", sAnimSelected);
...
float obj_AnimalDummy::DrawPropertyEditor(float scrx, float scry, float scrw, float scrh, const AClass* startClass, const GameObjects& selected)
{
	...
	{
		float flh = 250;
		r3dSTLString sDirFind = "Data\\ObjectsDepot\\G3_Animals\\Animations\\hunzi_*.anm";
...
void obj_AnimalDummy::DoHit()
{
	...
	char buf[MAX_PATH];
	sprintf(buf, "Data\\ObjectsDepot\\G3_Animals\\Animations\\%s", hitAnim);
