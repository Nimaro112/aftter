...
{
	const char* cpMeshName = "Data\\ObjectsDepot\\G3_Doors\\Door.sco";
...
{
	m_ActionUI_Title = gLangMngr.getString("$FR_OpenDoor");
...
BOOL obj_Door::Update()
{
	...
	if (m_OpenDoor==true)
	{
		m_ActionUI_Title = gLangMngr.getString("$FR_CloseDoor");
...
void obj_Door::SelectionDoor(int Selection)
{
	...
	{
		case 0:
			parent::MeshLOD[0] = r3dGOBAddMesh("Data\\ObjectsDepot\\G3_Doors\\Door.sco");
			break;
		case 1:
			parent::MeshLOD[0] = r3dGOBAddMesh("Data\\ObjectsDepot\\G3_Doors\\Door2.sco");
			break;
		case 2:
			parent::MeshLOD[0] = r3dGOBAddMesh("Data\\ObjectsDepot\\G3_Props_Military\\Mil_Wall_WireFenceBarb_R_01.sco");
	}
}

void obj_Door::ExeParticle()
{
	SpawnImpactParticle(r3dHash::MakeHash(""), r3dHash::MakeHash("Door_Smash_Wood"), GetPosition(), r3dPoint3D(0,1,0));
}
...
{
	obj_Door * newSpawnPoint = static_cast<obj_Door*>(srv_CreateGameObject("obj_Door", "Data\\ObjectsDepot\\G3_Doors\\Door.sco", GetPosition()));
...
float obj_Door::DrawPropertyEditor(float scrx, float scry, float scrw, float scrh, const AClass* startClass, const GameObjects& selected)
{
	...
	y += imgui_Value_SliderI(scrx, y, "Door Number", &SelectDoor, 0, 2, "%d");
