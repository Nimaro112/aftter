...
{
	const char* mesh = "Data\\ObjectsDepot\\TS_Tools\\air_uav_cypher2.sco";
	const char* skel = "Data\\ObjectsDepot\\TS_Tools\\Air_Uav_Cypher2.skl";
	if(!parent::Load(mesh)) 
...
BOOL obj_UAV::OnCreate()
{
	...
	int aid = m_AnimPool.Add("defaultanim", "Data\\ObjectsDepot\\TS_Tools\\Animations\\Air_Uav_Cypher2_PropsFly_01.anm");
	...
	{
		UavSnd = SoundSys.Play(SoundSys.GetEventIDByPath("Sounds/AM_Group/Item_Action/UAV_Movement_Loop_01"),GetPosition());
	...
	if(owner)
	{
		...
		if(!m_TagIcon.IsUndefined())
		{
			...
			if(CGL.localPlayer_)
			{
				...
				if( (dist < 20.0f) || (this->GetSafeID() == m_LocalPlayer_CurrentAimAt)
					)
				{
					showName = true;
				}
	...
	switch(n.state)
	{
		...
		case 2: // dead
			...
			if (SoundSys.isPlaying(UavSnd))
			{
				...
				SoundSys.Play(SoundSys.GetEventIDByPath("Sounds/AM_Group/Item_Action/UAV_Destruction_00"),GetPosition());
				SoundSys.Play(SoundSys.GetEventIDByPath("Sounds/AM_Group/Item_Action/UAV_Destruction_01"),GetPosition());
				...
				if (GetNetworkID() == gClientLogic().localPlayer_->UavNid_)
				{
					...
					for(int i=0; i<gClientLogic().localPlayer_->CurLoadout.BackpackSize; i++)
					{
						...
						if (wi.itemID == WeaponConfig::ITEMID_SpyDrone)
						{
							wi.Reset();
