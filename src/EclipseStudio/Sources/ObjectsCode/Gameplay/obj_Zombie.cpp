...
#endif
{
	...
	if(g_zombieBindSkeleton[0] == NULL) 
	{
		...
		g_zombieBindSkeleton[0]->LoadBinary("Data\\ObjectsDepot\\CharactersNew\\ProperScale_AndBiped.skl");
	...
	if(g_zombieBindSkeleton[1] == NULL) 
	{
		...
		g_zombieBindSkeleton[1]->LoadBinary("Data\\ObjectsDepot\\CharacterZombie\\Super_Zombie.skl");
	...
	if(g_zombieBindSkeleton[2] == NULL) 
	{
		...
		g_zombieBindSkeleton[2]->LoadBinary("Data\\ObjectsDepot\\G3_Animals\\Zombie_Dog.skl");
...
BOOL obj_Zombie::OnCreate()
{
	...
	if ( IsDogZombie() )
	{
		...
		if ( variant == 0 )
			strcpy(AnimalSCO,"Data\\ObjectsDepot\\G3_Animals\\zdog_10_body_01.sco");
		else
			sprintf(AnimalSCO,"Data\\ObjectsDepot\\G3_Animals\\zdog_0%i_body_01.sco",variant);
...
void obj_Zombie::InitPhysSkeletonCache( float progressStart, float progressEnd )
{
	...
	if( PhysSkeletonCache.Count() < PHYS_SKELETON_CACHE_SIZE )
	{
		...
		for( int i = 0; i < PHYS_SKELETON_CACHE_SIZE; i ++ )
		{
			...
			PhysSkeletonCache[ i ].skeleton = game_new r3dPhysSkeleton( "data/ObjectsDepot/CharactersNew/RagDoll.RepX" );
			PhysSkeletonCache[ i ].skeletonDog = game_new r3dPhysDogSkeleton( "data/ObjectsDepot/G3_Animals/ZDog.RepX" );
