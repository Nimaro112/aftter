...
class SkinRecipeConfig : public ModelItemConfig
{
public:
	SkinRecipeConfig(uint32_t id) : ModelItemConfig(id),numItemSkins(0) {}

	virtual bool loadBaseFromXml(pugi::xml_node& xmlItem);

	static const int MAX_SKINS=99;
	struct DefaultSkin
	{
		uint32_t SkinItemID;
		uint32_t itemSknID;
		uint32_t OriginalItemID;
		DefaultSkin() {};
		DefaultSkin(uint32_t w, uint32_t s, uint32_t o): itemSknID(w), SkinItemID(s), OriginalItemID(o) {}
	};
	DefaultSkin ItemSKN;

	uint32_t numItemSkins;
};
