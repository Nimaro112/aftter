...
IMPLEMENT_CLASS(obj_Dropped, "obj_Dropped", "Object");
AUTOREGISTER_CLASS(obj_Dropped);
...
BOOL obj_Dropped::Load(const char *fname)
{
	return TRUE;
}

BOOL obj_Dropped::OnCreate()
{
	r3d_assert(m_ItemID > 0);
	const char* cpMeshName = "";
	if(m_ItemID == WeaponConfig::ITEMID_Backpack1)
		cpMeshName = "Data\\ObjectsDepot\\SS_Loot\\SS_Loot_Corpse_Civ_01.sco";
		...
		cpMeshName = "Data\\ObjectsDepot\\SS_Loot\\SS_Loot_Corpse_Mil_01.sco";
		...
		cpMeshName = "Data\\ObjectsDepot\\SS_Loot\\SS_Loot_Corpse_Specops_01.sco";
	...
	m_ActionUI_Title = gLangMngr.getString("$FR_OPEN_CASE");
	m_ActionUI_Msg = gLangMngr.getString("HoldEToOpen");
	...
	// ptumik: do not do ray cast, as all the checks were done when placing Dropped, and what can happen now is that Dropped will be too far away from where server think it is and it will cause issues
	PxRaycastHit hit;
...
BOOL obj_Dropped::OnDestroy()
{
	return parent::OnDestroy();
}

BOOL obj_Dropped::Update()
{
	return parent::Update();
}

struct DroppedDebugRenderable : Renderable
{
	void Init()
	{
		DrawFunc = Draw;
	...
	{
		DroppedDebugRenderable* This = static_cast<DroppedDebugRenderable*>( RThis );
...
void obj_Dropped::AppendRenderables( RenderArray ( & render_arrays  )[ rsCount ], const r3dCamera& Cam ) 
{
	parent::AppendRenderables(render_arrays, Cam);

#ifndef FINAL_BUILD
	if(d_debug_show_dropped_owner->GetBool())
	{
		DroppedDebugRenderable rend;
