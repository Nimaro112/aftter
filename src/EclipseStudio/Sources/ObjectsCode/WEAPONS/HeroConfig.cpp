...
void HeroConfig::resetMesh()
{
	for(size_t i=0; i<m_LegsMeshes.size(); ++i)
		m_LegsMeshes[i] = NULL;
	for(size_t i=0; i<m_HairMeshes.size(); ++i) // hair system
		m_HairMeshes[i] = NULL; // hair system
...
r3dMesh* HeroConfig::getHairMesh(int index) const  // hair system
{
	...
	if(m_HairMeshes[index] == 0)
	{
		...
		{
			r3dError("ART: failed to load hair mesh '%s'\n", tmpBuf);
		}
...
r3dMesh* HeroConfig::getHeadMesh(int index) const
{
	...
	if(m_HeadMeshes[index] == 0)
	{
		...
		{
			r3dError("ART: failed to load head mesh '%s'\n", tmpBuf);
		}
...
r3dMesh* HeroConfig::getLegMesh(int index) const
{
	...
	if(m_LegsMeshes[index] == 0)
	{
		...
		{
			r3dError("ART: failed to load legs mesh '%s'\n", tmpBuf);
		}
...
r3dMesh* HeroConfig::getBodyMesh(int index, bool isFPS) const
{
	...
	if(!isFPS)
	{
		...
		if(m_BodyMeshes[index] == 0)
		{
			...
			{
				r3dError("ART: failed to load body mesh '%s'\n", tmpBuf);
			}
	...
	else
	{
		...
		if(m_BodyMeshesFPS[index] == 0)
		{
			...
			{
				r3dError("ART: failed to load fps body mesh '%s'\n", tmpBuf);
			}
...
size_t HeroConfig::getNumHairs() const // hair system
{
...
bool HeroConfig::loadBaseFromXml(pugi::xml_node& xmlHero)
{
	...
	int numHairs = xmlHero.child("HeroDesc").attribute("maxHairs").as_int(); // hair system
	...
	r3d_assert(m_HairMeshes.size() == 0); // hair system
