...
,ActualSkin(0)
{
	...
	if(m_pConfig->m_PrimaryAmmo->getShellExtractParticleName() && strlen(m_pConfig->m_PrimaryAmmo->getShellExtractParticleName())>2)
	{
		...
		TotalSkins = 0;
		GetSkins();// Weapon Skin
		if (TotalSkins>0)
			ChangeWeaponSKIN(0);// Weapon Skins
}
...
void Weapon::ChangeWeaponSKIN(int SkinID)
{
	ActualSkin=SkinID;
}

r3dMesh* Weapon::getMeshSkin(bool isFirstPerson)
{
	if (this == NULL)
		return NULL;
	...
	if (TotalSkins>0)
	{
		const BaseItemConfig* SkinCfg = g_pWeaponArmory->getConfig(Skins[ActualSkin].itemID);
		if (SkinCfg)
		{
			if (isFirstPerson)
			{
				char tmpStr[512];
				r3dscpy(tmpStr, SkinCfg->m_ModelFile);
				r3dscpy(&tmpStr[strlen(tmpStr)-4], "_FPS.sco");
				return r3dGOBAddMesh(tmpStr, true, false, true, true );
			}
			else
			return r3dGOBAddMesh(SkinCfg->m_ModelFile, true, false, true, true );
		}
	}
	return m_pConfig->getMesh(true, isFirstPerson);
}

void Weapon::SetSkins(int SkinsID, int itemID, char* StoreIcon)
{
	Skins[SkinsID].itemID = itemID;
	Skins[SkinsID].StoreIcon = StoreIcon;
	TotalSkins = SkinsID+1;
}

char* Weapon::GetIconSkin(int SkinID)
{
	if (SkinID<TotalSkins)
		return Skins[SkinID].StoreIcon;
	else
		return "Not found";
}

bool Weapon::GetSkins()
{
		const char* SkinDBFile = "Data/Weapons/itemsDB.xml";
		...
		pugi::xml_node xmlSkinSystem = xmlFile.child("DB");
	{
		pugi::xml_node xmlSkinsArmory = xmlSkinSystem.child("WeaponSkinArmory");
		uint32_t itemID = getItemID();
		pugi::xml_node xmlSkinWeapon = xmlSkinsArmory.child("Weapon");
		while(!xmlSkinWeapon.empty())
		{
			...
			if (itemID!=0)
			{
				...
				uint32_t itemIDSkin = xmlSkinWeapon.attribute("itemID").as_uint();
				if (itemIDSkin == itemID)
				{
					const BaseItemConfig* Defaultconfig = g_pWeaponArmory->getConfig(itemID);
					if (Defaultconfig)
					{
						r3dMesh* SkinMesh = r3dGOBAddMesh(Defaultconfig->m_ModelFile, true, false, true, true );// true, false, true, true );

						if (SkinMesh != 0)
							SetSkins(0,itemID,Defaultconfig->m_StoreIcon);
						else
							r3dOutToLog("##### This Model not is correct %s\n",Defaultconfig->m_ModelFile);

						char tmpStr[512];
						r3dscpy(tmpStr, Defaultconfig->m_ModelFile);
						r3dscpy(&tmpStr[strlen(tmpStr)-4], "_FPS.sco");

						r3dMesh* SkinMeshFPS = r3dGOBAddMesh(tmpStr, true, false, true, true );
					...
					for (int i=0;i<99;i++)
					{
						char buf[64];
						sprintf(buf, "sk%d", i);
						uint32_t SKINID = xmlSkinWeapon.attribute(buf).as_uint();
						...
						if (config != NULL)
						{
							...
							if (SkinMesh != 0)
								SetSkins(SkinsFound,SKINID,config->m_StoreIcon);
							else
								r3dOutToLog("##### This Model not is correct %s\n",config->m_ModelFile);

							char tmpStr[512];
							r3dscpy(tmpStr, config->m_ModelFile);
							r3dscpy(&tmpStr[strlen(tmpStr)-4], "_FPS.sco");
							...
							r3dOutToLog("##### This Skin not is correct %d\n",SKINID);
							SkinsFound++;
						}
					}				
				}
			...
			xmlSkinWeapon = xmlSkinWeapon.next_sibling();
		}
	}
	return false;
}
