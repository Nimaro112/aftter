...
bool WeaponArmory::Init()
{
	...
	{
		pugi::xml_node xmlItems = xmlDB.child("ConsumablesArmory");
		pugi::xml_node xmlItem = xmlItems.child("Item");
		while(!xmlItem.empty())
		{
	...
	{
		pugi::xml_node xmlItems = xmlDB.child("SkinRecipeArmory");
		pugi::xml_node xmlItem = xmlItems.child("Item");
		while(!xmlItem.empty())
		{
			loadSkinRecipeItem(xmlItem);
			xmlItem = xmlItem.next_sibling();
		}
	}
	// delete only after we are done parsing xml!
...
SkinRecipeConfig* WeaponArmory::loadSkinRecipeItem(pugi::xml_node& xmlItem)
{
	r3d_assert(!xmlItem.empty());
	...
	SkinRecipeConfig* recipe = game_new SkinRecipeConfig(itemID);
	recipe->loadBaseFromXml(xmlItem);

	m_itemsHash.Add(itemID, recipe);
	m_NumSkinsRecipesLoaded++;
...
const SkinRecipeConfig* WeaponArmory::getSkinRecipeConfig(uint32_t itemID)
{
	BaseItemConfig* item = NULL;
	if(m_itemsHash.GetObject(itemID, &item))
	{
		if(item->category == storecat_Skins)
			return (SkinRecipeConfig*)item;
	}
	return NULL;
}
...
const GearConfig* WeaponArmory::getGearConfig(uint32_t itemID)
{
	...
	if(m_itemsHash.GetObject(itemID, &item))
	{
		if(item->category == storecat_Armor || item->category == storecat_Helmet || item->category == storecat_Mask || item->category == storecat_Scarf)
