...
bool WeaponAttachmentConfig::loadBaseFromXml(pugi::xml_node& xmlAttachment)
{
	...
	m_DurabilityUse = xmlAttachment.child("ItemDurability").attribute("DurabilityUse").as_float();
	m_RepairAmount = xmlAttachment.child("ItemDurability").attribute("RepairAmount").as_float();
	m_PremRepairAmount = xmlAttachment.child("ItemDurability").attribute("PremRepairAmount").as_float();
	m_RepairPriceGD = xmlAttachment.child("ItemDurability").attribute("RepairPriceGD").as_int();
...
bool WeaponConfig::loadBaseFromXml(pugi::xml_node& xmlWeapon)
{
	...
	m_DurabilityUse = xmlWeapon.child("ItemDurability").attribute("DurabilityUse").as_float();
	m_RepairAmount = xmlWeapon.child("ItemDurability").attribute("RepairAmount").as_float();
	m_PremRepairAmount = xmlWeapon.child("ItemDurability").attribute("PremRepairAmount").as_float();
	m_RepairPriceGD = xmlWeapon.child("ItemDurability").attribute("RepairPriceGD").as_int();
	if(m_DurabilityUse > 0) m_DurabilityUse = 100.0f / m_DurabilityUse; // convert from number of uses to percent per use
