...
void getWeaponStats(const WeaponConfig* wc, float* damagePerc, int* damage, float* spreadPerc, int* spread, float* recoilPerc, int* recoil, float* decayPerc, int* decay) // Weapons Stats bar
{	
...
void getGearStats(const GearConfig* gc, float* damagePerc, int* damage) // Gear Stats bar
{	
		const float maxScaleValue = 0.61f;
...
void getAttachmentStats(const WeaponAttachmentConfig* attc, float* damagePerc, int* damage, float* spreadPerc, int* spread, float* recoilPerc, int* recoil) // Attachments Stats bar
{	
		const float maxScaleValue = 0.61f;
		const int maxDamageValue = 100;		
		const int maxSpreadValue = 15;
...
void getAmmoStats(const WeaponAttachmentConfig* ammoc, float* damagePerc, int* damage, float* spreadPerc, int* spread, float* recoilPerc, int* recoil, float* clipSizePerc, int* clipsize) // Ammo Stats bar
{	
		const float maxScaleValue = 0.61f;
		const int maxDamageValue = 100;
...
void getBackpackStats(const BackpackConfig* bc,  float* maxSlotsPerc, int* maxSlots, float* maxWeightPerc, int* maxWeight) // Backpack Stats bar
{	
		const float maxScaleValue = 0.61f;
...
void getFoodStats(const FoodConfig* fc, float* healthPerc, int* health, float* toxicityPerc, int* toxicity, float* waterPerc, int* water, float* foodPerc, int* food, float* staminaPerc, int* stamina) // Food Stats bar
{
		const float maxScaleValue = 0.61f;
...
void addItemCategoriesToUI(class r3dScaleformMovie& gfxMovie)
{
	...
	var[0].SetNumber(4);
	var[1].SetString("cosmetic");
	var[2].SetBoolean(true);
	var[3].SetBoolean(true);
	gfxMovie.Invoke("_root.api.addTabType", var, 4);

	var[0].SetNumber(5);
	var[1].SetString("guns");
	var[2].SetBoolean(true);
	var[3].SetBoolean(true);
	gfxMovie.Invoke("_root.api.addTabType", var, 4);
...
void addItemsAndCategoryToUI(r3dScaleformMovie& gfxMovie)
{
	...
	r3dgameVector(const SkinRecipeConfig*) allSkins;
	...
	for(size_t i = 0; i < heroSize; ++i)
	{
		...
		var[7].SetInt(hero->getNumHairs()); // hair system
		var[8].SetInt(hero->getNumHeads());
		var[9].SetInt(hero->getNumBodys());
		var[10].SetInt(hero->getNumLegs());
		gfxMovie.Invoke("_root.api.addHero", var, 11);
	...
	for(size_t i = 0; i < weaponSize; ++i)
	{
		...
		/////////////////////////////////////////////////////
		{
			...
			if(weapon->m_itemID == WeaponConfig::ITEMID_Bandages 
			|| weapon->m_itemID == WeaponConfig::ITEMID_Medkit 
			|| weapon->m_itemID == WeaponConfig::ITEMID_Suture_Kit
			|| weapon->m_itemID == WeaponConfig::ITEMID_Homebrew_Bandage)
			{				
				var[1].SetString(gLangMngr.getString("StatHealth"));
				var[2].SetNumber(damage);
				var[3].SetNumber(damagPerc);
			...
			if(weapon->m_itemID == WeaponConfig::ITEMID_C01Vaccine)
			{				
				var[1].SetString(gLangMngr.getString("StatToxity"));
				var[2].SetNumber(damage);
				var[3].SetNumber(damagPerc);
	...
	// categories
	var[0].SetNumber(storecat_ASR); // ASSAULT RIFLE CATEGORY
	var[1].SetString("storecat_ASR");
	var[2].SetNumber(5);
	var[3].SetNumber(0);
	gfxMovie.Invoke("_root.api.addCategory", var, 4);

	var[0].SetNumber(storecat_SNP); // SNIPER CATEGORY
	var[1].SetString("storecat_SNP");
	var[2].SetNumber(5);
	var[3].SetNumber(0);
	gfxMovie.Invoke("_root.api.addCategory", var, 4);

	var[0].SetNumber(storecat_SHTG); // SHOTGUN CATEGORY
	var[1].SetString("storecat_SHTG");
	var[2].SetNumber(5);
	var[3].SetNumber(0);
	gfxMovie.Invoke("_root.api.addCategory", var, 4);

	var[0].SetNumber(storecat_MG); // MACHINE GUN CATEGORY
	...
	var[0].SetNumber(storecat_SMG); //SUB MACHINE GUN CATEGORY
	var[1].SetString("storecat_SMG");
	var[2].SetNumber(5);
	var[3].SetNumber(0);
	gfxMovie.Invoke("_root.api.addCategory", var, 4);

	var[0].SetNumber(419);// AMMO CATEGORY
	var[1].SetString("ammo"); 
	var[2].SetNumber(3); 
	var[3].SetNumber(-1);
	gfxMovie.Invoke("_root.api.addCategory", var, 4);

	var[0].SetNumber(storecat_FPSAttachment); // ATTACHMENT - SCOPE CATEDORY
	var[1].SetString("storecat_FPSAttachment"); 
	var[2].SetNumber(0); 
	var[3].SetNumber(-1);
	gfxMovie.Invoke("_root.api.addCategory", var, 4);
	
	var[0].SetNumber(storecat_GRENADE); // GRENADES CATEGORY
	var[1].SetString("storecat_GRENADE"); 
	var[2].SetNumber(3);
	var[3].SetNumber(4);
	gfxMovie.Invoke("_root.api.addCategory", var, 4);

	var[0].SetNumber(storecat_MELEE); // MELEE - KNIFE CATEGORY
	var[1].SetString("storecat_MELEE");
	var[2].SetNumber(5);
	var[3].SetNumber(1);
	gfxMovie.Invoke("_root.api.addCategory", var, 4);
	
	var[0].SetNumber(storecat_Armor); // ARMOR - GEAR CATEGORY
	var[1].SetString("storecat_Armor"); 
	var[2].SetNumber(0);
	var[3].SetNumber(2);
	gfxMovie.Invoke("_root.api.addCategory", var, 4);

	var[0].SetNumber(storecat_Helmet); //HATS - HELMET CATEGORY
	var[1].SetString("storecat_Helmet"); 
	var[2].SetNumber(4);
	var[3].SetNumber(3);
	gfxMovie.Invoke("_root.api.addCategory", var, 4);

	var[0].SetNumber(storecat_Mask); //SCARF - SCARF HEAD CATEGORY
	var[1].SetString("storecat_Mask"); 
	var[2].SetNumber(4); // MarketPlace Tab
	var[3].SetNumber(3); // hud slot
	gfxMovie.Invoke("_root.api.addCategory", var, 4);

	var[0].SetNumber(storecat_Scarf); //SCARF - SCARF HEAD CATEGORY
	var[1].SetString("storecat_Scarf"); 
	var[2].SetNumber(4);
	var[3].SetNumber(3); // hud slot
	gfxMovie.Invoke("_root.api.addCategory", var, 4);

	var[0].SetNumber(storecat_Backpack); // BACKPACK CATEGORY
	var[1].SetString("storecat_Backpack"); 
	var[2].SetNumber(0);
	var[3].SetNumber(-1);
	gfxMovie.Invoke("_root.api.addCategory", var, 4);

	var[0].SetNumber(storecat_Food); // FOOD CATEGORY
	var[1].SetString("storecat_Food");
	var[2].SetNumber(1);
	var[3].SetNumber(-1);
	gfxMovie.Invoke("_root.api.addCategory", var, 4);

	var[0].SetNumber(storecat_Water); // WATER CATEGORY
	var[1].SetString("storecat_Water");
	var[2].SetNumber(1);
	var[3].SetNumber(-1);
	gfxMovie.Invoke("_root.api.addCategory", var, 4);

	var[0].SetNumber(storecat_UsableItem); // ALL USABLE ITEM CATEGORY
	var[1].SetString("storecat_UsableItem");
	var[2].SetNumber(2);
	var[3].SetNumber(4);
	gfxMovie.Invoke("_root.api.addCategory", var, 4);
	
	var[0].SetNumber(storecat_Components); // CRAFTING COMPONENTS CATEGORY
	var[1].SetString("storecat_Components");
	var[2].SetNumber(7);
	var[3].SetNumber(-1);
	gfxMovie.Invoke("_root.api.addCategory", var, 4);

	var[0].SetNumber(storecat_CraftRecipe); // CRAFT RECIPE - CARD - BLUEPRINTS  CATEGORY
	var[1].SetString("storecat_CraftRecipe");
	var[2].SetNumber(2);
	var[3].SetNumber(-1);
	gfxMovie.Invoke("_root.api.addCategory", var, 4);
	
	var[0].SetNumber(storecat_Skins);
    var[1].SetString("storecat_Skins");
	var[2].SetNumber(6);
	var[3].SetNumber(-1);
    gfxMovie.Invoke("_root.api.addCategory", var, 4);
	
	var[0].SetNumber(storecat_LootBox); // CASE SYSTEM CATEGORY
    var[1].SetString("storecat_LootBox");
    var[2].SetNumber(7);
    var[3].SetNumber(-1);
    gfxMovie.Invoke("_root.api.addCategory", var, 4);

	var[0].SetNumber(storecat_HeroPackage); // HERO CATEGORY
...
const char* getAdditionalDescForItem(uint32_t itemID, int Var1, int Var2, int Var3)
{
	...
	if(wc && wc->category != storecat_UsableItem && wc->category != storecat_GRENADE && wc->category!=storecat_MELEE) // weapon
    {
        ...
        if(clip)
        {
            ...
            if(Var1<0)
                sprintf(first_string, gLangMngr.getString("WeaponInfoString_BrandNew"), Damage, Spread, Recoil);
            else if(ammoLeft > 0)
            {
                if(ammoLeft == 1)
