...
void getWeaponStats(const class WeaponConfig* wc, float* damagePerc, int* damage, float* spreadPerc, int* spread, float* recoilPerc, int* recoil, float* decayPerc, int* decay);

void getGearStats(const class GearConfig* gc, float* damagePerc, int* damage);
void getAttachmentStats(const class WeaponAttachmentConfig* attc, float* damagePerc, int* damage, float* spreadPerc, int* spread, float* recoilPerc, int* recoil);
void getAmmoStats(const class WeaponAttachmentConfig* ammoc, float* damagePerc, int* damage, float* spreadPerc, int* spread, float* recoilPerc, int* recoil, float* clipSizePerc, int* clipsize);
void getBackpackStats(const class BackpackConfig* bc, float* maxSlotsPerc, int* maxSlots, float* maxWeightPerc, int* maxWeight);
void getFoodStats(const class FoodConfig* fc, float* healthPerc, int* health, float* toxicityPerc, int* toxicity, float* waterPerc, int* water, float* foodPerc, int* food, float* staminaPerc, int* stamina);
