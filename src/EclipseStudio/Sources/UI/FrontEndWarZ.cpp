...
{
	if(mapID == GBGameInfo::MAPID_AM_Island) return gLangMngr.getString("MapIsland");
	else if(mapID == GBGameInfo::MAPID_AM_Battleroyale) return gLangMngr.getString("MapBr");
	else if(mapID == GBGameInfo::MAPID_AM_Compound) return gLangMngr.getString("MapCompound");
	else if(mapID == GBGameInfo::MAPID_AM_ConventionC) return gLangMngr.getString("MapConvention");
	else if(mapID == GBGameInfo::MAPID_AM_Devmap) return gLangMngr.getString("MapDevmap");
	else if(mapID == GBGameInfo::MAPID_AM_PineTop) return gLangMngr.getString("MapPinetop");
	else if(mapID == GBGameInfo::MAPID_AM_RoTB) return gLangMngr.getString("MapRoTB");
	else if(mapID == GBGameInfo::MAPID_AM_Nevada) return gLangMngr.getString("MapNevada");
	else if(mapID == GBGameInfo::MAPID_AM_SouthWest) return gLangMngr.getString("MapSouthWest");
	else if(mapID == GBGameInfo::MAPID_AM_RentServer01) return gLangMngr.getString("MapCompoundRent");
	else if(mapID == GBGameInfo::MAPID_AM_RentServer02) return gLangMngr.getString("MapConventionRent");
...
, r3dIResource(r3dIntegrityGuardian())
{
	...
	leaderboardSize = 1000; // max member in leaderboard
...
static void LoadFrontendGameData(FrontendWarZ* UI)
{
	...
	{
		PFX_3DLUTColorCorrection::s_damageHitColorCorrectionVal = 0.0f; // don't show color correction when player at low life perc
		obj_Player* plr = (obj_Player *)srv_CreateGameObject("obj_Player", "Player", r3dPoint3D(0,0,0));
		plr->PlayerState = PLAYER_IDLE; // player state animation
		plr->bDead = 0; // don't show ragdoll at frontend
		plr->CurLoadout = gUserProfile.ProfileData.ArmorySlots[0]; // player slots
		plr->m_disablePhysSkeleton = true; // disable phyx of the skeleton
		plr->m_fPlayerRotationTarget = plr->m_fPlayerRotation = 0; // player rotation - untest
		...
		plr->uberAnim_->IsInUI = true; // default idl animation at frontend
...
#define MAKE_CALLBACK(FUNC) game_new r3dScaleformMovie::TGFxEICallback<FrontendWarZ>(this, &FrontendWarZ::FUNC)
	...
	gfxMovie.RegisterEventHandler("eventShowInventory", MAKE_CALLBACK(eventShowInventory));	
...
void FrontendWarZ::postLoginStepInit(EGameResult gameResult)
{
	...
	r3dGameLevel::SetHomeDir("FrontEnd_Render");
	r3dGameLevel::SetGassEnable("Frontend",true);
...
//typedef VOID (* GHPSetParam) (DWORD APIKey, UINT id, UCHAR * buffer, UINT size); // GHP ANTICHEAT
//typedef VOID (* GHPGetParam) (DWORD APIKey, UINT id, UCHAR * buffer, UINT size); // GHP ANTICHEAT
...
int FrontendWarZ::Update()
{
	...
	if(handleGetProfileData != 0)
	{
		...
		// GHP ANTICHEAT
		//HMODULE hGHP = GetModuleHandle("godray.DLL");
		...
		if(hGHP != NULL)
		{
			...
			if (SetParam != NULL && GetParam != NULL)
			{
				...
				DWORD APIKey = 0x512B9EA7;
				DWORD Time_1 = 0x3844F7FD;
				DWORD Time_2 = 0x8EA1285D; 
			...
			{
                MessageBoxA(0,"AntiCheat ERROR!","DLL OR LICENSE INVALID!",MB_OK | MB_ICONSTOP);
				ExitProcess(0); 
			}
		}*/
		// GHP ANTICHEAT
	...
	if(gUserProfile.ProfileData.ArmorySlots[gUserProfile.SelectedCharID].Alive == 0 && gUserProfile.ProfileData.ArmorySlots[gUserProfile.SelectedCharID].Hardcore == 0) // dead
	{
		...
		const int timeToReviveInSec = 1; // 10 and 20 min
...
//#endif
	...
	m_Player->Darkness = 0; // show dead player as darkness frame
	...
	r3dPoint3D playerPosHome(0, 0.38f, 0.50f);
	r3dPoint3D playerPosCreate(0, 0.38f, 0.50f);
	...
	if(m_needPlayerRenderingRequest==1) // home // Player frontend camera
	...
	//// Yuri-BR Player Rotation
	 int mMX=Mouse->m_MouseMoveX;    float glb_MouseSensAdj = 0.60f;  
	 float mmoveX = float(-mMX) * glb_MouseSensAdj;    
	 if(Mouse->IsPressed(r3dMouse::mLeftButton))    
	 {        
		 plrotation += mmoveX;         
		 if(plrotation > 360)  plrotation = 360;        
		 if(plrotation < -360) plrotation = -360;    
	 }
	m_Player->m_fPlayerRotationTarget = m_Player->m_fPlayerRotation = plrotation;
	//// Yuri-BR Player Rotation
...
void FrontendWarZ::drawPlayer()// Player frontend camera
{
...
void FrontendWarZ::addClientSurvivor(const wiCharDataFull& slot, int slotIndex)
{
	...
	var[6].SetNumber(slot.HairIdx); // hair system
	...
	var[17].SetNumber(slot.Stats.KilledZombies);		// zombies Killed
	var[18].SetNumber(slot.Stats.KilledBandits);		// bandits killed
	var[19].SetNumber(slot.Stats.KilledSurvivors);		// civilians killed
	char repS[64];
	sprintf(repS, "%s : %d", getReputationString(slot.Stats.Reputation), slot.Stats.Reputation);
	var[20].SetString(repS);	// alignment

	const char* lastMap = getMapName(slot.GameMapId);
	var[21].SetString(lastMap);

	var[22].SetBoolean(slot.GameFlags & wiCharDataFull::GAMEFLAG_NearPostBox);
	var[23].SetInt(slot.Stats.XP - slot.Stats.SpendXP);
	/*char kdrS[32];
	sprintf(kdrS, "%.2f", getRatio(float(slot.Stats.KilledSurvivors + slot.Stats.KilledBandits), float(slot.Stats.Deaths)));
	var[24].SetString(kdrS); // kdr
	char hsrS[32];
	sprintf(hsrS, "%.2f", getRatio(float(slot.Stats.ShotsHeadshots), float(slot.Stats.ShotsFired)));
	var[25].SetString(hsrS); // hsr
	var[26].SetInt(int(getRatio((float)slot.Stats.ShotsHits, (float)slot.Stats.ShotsFired)*100.0f)); // accuracy
	var[27].SetInt(slot.Stats.Deaths); // died*/

	gfxMovie.Invoke("_root.api.addClientSurvivor", var, 28);
...
void FrontendWarZ::UpdateSkinIcons(const wiCharDataFull& slot, bool OriginalSkin)
{
		for (int b = 0; b < slot.BackpackSize; b++)
		{	
			if (slot.Items[b].itemID != 0)
			{
				const GearConfig* gc = g_pWeaponArmory->getGearConfig(slot.Items[b].itemID);
				const BaseItemConfig* itm = g_pWeaponArmory->getConfig(slot.Items[b].itemID);
				if (gc)
				{
					int SkinID = m_Player->GetItemIDSkin("Gear",slot.Items[b].itemID,m_Player->GetSkinIDHud(slot.Items[b].itemID));
					...
					if (bic)
					{
						...
						if (OriginalSkin)
							var[1].SetString(itm->m_StoreIcon);
						else
				...
				if (itm)
				{
					...
					{
						int SkinID = m_Player->GetItemIDSkin("Weapon",slot.Items[b].itemID,m_Player->GetSkinIDHud(slot.Items[b].itemID));
...
void FrontendWarZ::initFrontend()
{
	...
	// tutorial screens
	if(g_num_matches_played->GetInt() >= 4 )
	{
	...
	else if(g_num_matches_played->GetInt() <= 3 )
	{
		if(g_shown_frontend_tutorial->GetBool() == true)
		{
	...
	if(g_num_matches_played->GetInt() >= 4 )
	{
		if(g_shown_frontend_tutorial->GetBool() == false)
		{
			gfxMovie.SetVariable("_root.api.Main.SurvivorsAnim.Survivors.ChangeName.Tutorial.visible", false);
		}
	}
	else if(g_num_matches_played->GetInt() <= 3 )
	{
		if(g_shown_frontend_tutorial->GetBool() == true)
		{
			gfxMovie.SetVariable("_root.api.Main.SurvivorsAnim.Survivors.ChangeName.Tutorial.visible", true);
		}
	}
	...
	// add skill info
	{
		...
		for(int i=0; i<34; ++i)
		{
			...
			char iconPath[512];
			sprintf(iconPath, "$data/Interface/skillIcons/Skill%d.dds", i);
			var[3].SetString(iconPath);
			char iconPathBW[512];
			sprintf(iconPathBW, "$data/Interface/skillIcons/Skill%dBW.dds", i);
	...
	Scaleform::GFx::Value var[20]; //SERVERS CHANNELS - Server channels - Server channel
	// init browse channel buttons
	{
		var[0].SetBoolean(true); //HARDCORE
		var[1].SetBoolean(true); // Oficial Server
		var[2].SetBoolean(true); // private servers
		var[3].SetBoolean(gUserProfile.ShopPremiumAcc>0 || gUserProfile.ProfileData.PremiumAcc>0); // premium servers (enable only if premium is available for sale or player alreay has premium account
		var[4].SetBoolean(false); // strongholds server
		var[5].SetBoolean(true); // veteran servers
		var[6].SetBoolean(true); // public test environment 
	...
	//gfxMovie.SetVariable("_root.api.Main.SurvivorsAnim.Survivors.PopUpBrowser.visible", true);
	...
	{
		gfxMovie.Invoke("_root.api.addClanIcon", "$data/Interface/clanIcons/clan_survivor.dds");
		gfxMovie.Invoke("_root.api.addClanIcon", "$data/Interface/clanIcons/clan_bandit.dds");
		gfxMovie.Invoke("_root.api.addClanIcon", "$data/Interface/clanIcons/clan_lawman.dds");
		// new clan icons
		gfxMovie.Invoke("_root.api.addClanIcon", "$data/Interface/clanIcons/clan_warzone.dds");
		gfxMovie.Invoke("_root.api.addClanIcon", "$data/Interface/clanIcons/clan_target.dds");
		gfxMovie.Invoke("_root.api.addClanIcon", "$data/Interface/clanIcons/clan_time.dds");
	}
	{
	...
	{
		// Next Map on Rent Servers
		Scaleform::GFx::Value var[3];
		//var[0].SetInt(GBGameInfo::MAPID_AM_PineTop);
		var[1].SetString(gLangMngr.getString("CHOOSE YOUR MAP"));
		gfxMovie.Invoke("_root.api.addRentServer_MapInfo", var, 2);
		
		// Next Map on Rent Servers
		var[0].SetInt(GBGameInfo::MAPID_AM_RentServer01); // good
		var[1].SetString(gLangMngr.getString("MapCompoundRent"));
		gfxMovie.Invoke("_root.api.addRentServer_MapInfo", var, 2);
		
		// Next Map on Rent Servers
		var[0].SetInt(GBGameInfo::MAPID_AM_RentServer02); // good
		var[1].SetString(gLangMngr.getString("MapConvention"));
		gfxMovie.Invoke("_root.api.addRentServer_MapInfo", var, 2);
		
		// Next Map on Rent Servers
		var[0].SetInt(GBGameInfo::MAPID_AM_PineTop); // good
		var[1].SetString(gLangMngr.getString("PineTop"));
		gfxMovie.Invoke("_root.api.addRentServer_MapInfo", var, 2);

		//var[0].SetInt(GBGameInfo::MAPID_SR_Cliffside); // need id
...
{
	exitRequested_ = true;
	ShellExecute(NULL, "open", "http://playafterlifegame.com/", "", NULL, SW_SHOW); //OPEN INTERNET NAVEGATION WINDOWS FOR THE WEBSITE
}
...
void FrontendWarZ::eventCreateCharacter(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	m_CreateHairIdx 	= args[3].GetInt(); // hairID - hair system
	m_CreateHeadIdx 	= args[4].GetInt(); // headID
	m_CreateBodyIdx 	= args[5].GetInt(); // bodyID
	m_CreateLegsIdx 	= args[6].GetInt(); // legsID
	...
	if(gUserProfile.ProfileData.NumSlots >= 1) 
	{
		Scaleform::GFx::Value var[2];
...
void FrontendWarZ::eventBuyItem(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	mStore_BuyQuantity = args[3].GetInt(); // Quantity Option
	mStore_BuyPrice *= mStore_BuyQuantity; // Quantity Option
	mStore_BuyPriceGD *= mStore_BuyQuantity; // Quantity Option
...
bool FrontendWarZ::ConnectToMasterServer()
{
	...
	needReturnFromQuickJoin = false; /// bkp = true
...
unsigned int WINAPI FrontendWarZ::as_PlayGameThread(void* in_data)
{
	...
	if(This_->WaitForGameJoinAnswer() == false)
	{
		This_->SetAsyncError(0, gLangMngr.getString("TimeoutJoiningGame"));
			SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/AM_Group/FrontEnd/UI_Menu_Error"), r3dPoint3D(0,0,0));
		This_->needReturnFromQuickJoin = false;  /// bkp = true
		return 0;
	}
...
unsigned int WINAPI FrontendWarZ::as_CreateCharThread(void* in_data)
{
	...
	int apiCode = gUserProfile.ApiCharCreate(This->m_CreateGamerTag, This->m_CreateGameMode, This->m_CreateHeroID, This->m_CreateHairIdx, This->m_CreateHeadIdx, This->m_CreateBodyIdx, This->m_CreateLegsIdx);
	if(apiCode != 0)
	{
		if(apiCode == 9)
		{
...
{       
        if(gUserProfile.ProfileData.lastWinID > 0)
        {
                ...
                const BaseItemConfig* ic = g_pWeaponArmory->getConfig(gUserProfile.ProfileData.lastWinID);
				const BackpackConfig* gg = g_pWeaponArmory->getBackpackConfig(gUserProfile.ProfileData.lastWinID);
                ...
                if(wc)
                {
					...
					sprintf(winmsg, "<font color=\"#ed0000\">%s</font>", wc->m_StoreName);
                ...
                else if(gc)
				{
					...
					gfxMovie.Invoke("_root.api.showInfoMsgCase", var, 1);
					sprintf(winmsg, "<font color=\"#ed0000\">%s</font>", gc->m_StoreName);
                ...
                else if(fc)
                {
					...
					gfxMovie.Invoke("_root.api.showInfoMsgCase", var, 1);
					sprintf(winmsg, "<font color=\"#ed0000\">%s</font>", fc->m_StoreName);
                ...
                else if(ic)
                {
					...
					gfxMovie.Invoke("_root.api.showInfoMsgCase", var, 1);
					sprintf(winmsg, "<font color=\"#ed0000\">%s</font>", ic->m_StoreName);
				...
				else if(gg)
                {
					...
					gfxMovie.Invoke("_root.api.showInfoMsgCase", var, 1);
					sprintf(winmsg, "<font color=\"#ed0000\">%s</font>", gg->m_StoreName);
                ...
                gfxMovie.Invoke("_root.api.showInfoMsgBuy", "");
...
bool FrontendWarZ::UpdateInventoryWithBoughtItem()
{
	...
	if(mStore_BuyItemID == 301586)//filter for lottery ticket ItemID
		return true;
...
void FrontendWarZ::eventShowInventory(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	UpdateSkinIcons(gUserProfile.ProfileData.ArmorySlots[gUserProfile.SelectedCharID],false);
}
...
void FrontendWarZ::eventBackpackFromInventory(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	if (itemID == 103018 && gUserProfile.ProfileData.PremiumAcc == 0)
	{
			Scaleform::GFx::Value var[3];
			var[0].SetString(gLangMngr.getString("$FR_PremiumAccountRequired"));
			SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/AM_Group/FrontEnd/PremiumRequirement"), r3dPoint3D(0,0,0));
			var[1].SetBoolean(true);
			var[2].SetString("");
			gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
			return;
...
void FrontendWarZ::OnBackpackFromInventorySuccess()
{
	...
	UpdateSkinIcons(gUserProfile.ProfileData.ArmorySlots[slot],false);
	gfxMovie.Invoke("_root.api.hideInfoMsg", "");
...
void FrontendWarZ::eventCreateChangeCharacter(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	PlayerCreateLoadout.HairIdx 	= args[1].GetInt(); // hair system
	PlayerCreateLoadout.HeadIdx 	= args[2].GetInt();
	PlayerCreateLoadout.BodyIdx 	= args[3].GetInt();
	PlayerCreateLoadout.LegsIdx 	= args[4].GetInt();
...
void FrontendWarZ::eventBrowseGamesJoin(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	if(gdata)
	{
		...
		if (gClientLogic().isBattleRoyaleMode == 1)//battleroyale
		{
				Scaleform::GFx::Value var[3];
				...
				var[2].SetString("");
				gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
				gClientLogic().isBattleRoyaleMode=0;
			return;
		}
...
void FrontendWarZ::processNewGameList()
{
	...
	for(int i=0; i<numGames; i++) 
	{
		...
		if(ginfo.gameType == 0)
		{
			serverType = "GAMEWORLD";
		}
		else if(ginfo.gameType == 1)
		{
			serverType = "BATTLE ROYALE";
		}
		else if(ginfo.gameType == 2)
		{
		...
		//var[2].SetString(ginfo.IsGameworld() ? "GAMEWORLD" : "DEATHMATCH");
		...
		if (ginfo.mapId != GBGameInfo::MAPID_AM_Devmap )
		{
...
void FrontendWarZ::ProcessSupervisorPings()
{
	for(size_t i = 0; i < gMasterServerLogic.supers_.size(); ++i)
	{
		...
		{
#ifndef FINAL_BUILD		
			r3dError("Too Many servers, please contact an admin.");
#endif
			continue;
		}
...
void FrontendWarZ::eventClanAdminAction(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	if(strcmp(actionType, "demote") == 0)
	{
		...
		else
		{
			...
			SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/AM_Group/FrontEnd/UI_Trade_Success"), r3dPoint3D(0,0,0));
			var[1].SetBoolean(true);
			gfxMovie.Invoke("_root.api.showInfoMsg", var, 2);
		}
	}
...
void FrontendWarZ::eventClanLeaveClan(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	{
		//BUG FIX: LEAVE CLAN AND NEED TO CLOSE GAME TO RELOAD INFO
		wiCharDataFull& slot = gUserProfile.ProfileData.ArmorySlots[gUserProfile.SelectedCharID];
		updateClientSurvivor(slot);
...
void FrontendWarZ::eventClanRespondToInvite(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	else if(isAccepted)
	{
		setClanInfo();
		gfxMovie.Invoke("_root.api.Main.showScreen", "MyClan");
		//BUG FIX: NEED TO CLOSE GAME TO RELOAD INFO
		wiCharDataFull& slot = gUserProfile.ProfileData.ArmorySlots[gUserProfile.SelectedCharID];
		updateClientSurvivor(slot);
	}
	else if(!isAccepted)
...
void FrontendWarZ::eventStorePurchaseGPRequest(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	// open up user panel
	sprintf(url, "http://playafterlifemmo.ddns.net/gamecp/index.php");
...
void FrontendWarZ::eventStorePurchaseGP(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	//	char url[256];
	//	//@TH
	//	sprintf(url, "http://playafterlifegame.com/account/buygc.php", authKey, howMuchGC);
	//	ShellExecute(NULL, "open", url, "", NULL, SW_SHOW);
	//}
	//// minimize our window
	//ShowWindow(win::hWnd, SW_MINIMIZE);
...
void FrontendWarZ::OnRequestLeaderboardSuccess()
{
	...
	gfxMovie.Invoke("_root.api.Main.LeaderboardAnim.clearLeaderboardList", "");
	{
		...
		for(size_t i=0; i<gUserProfile.m_lbData[leaderboard_requestTableID].size(); ++i)
		{
			...
			case 6:
				...
				break;
			case 7:
				sprintf(tmpStr, "%d", gUserProfile.m_lbData[leaderboard_requestTableID][i].stats.VictorysHardGames); //battleroyale
				break;
			default:
				r3d_assert(false);
				break;
	...
	case 6:
		...
		break;
	case 7:
		gfxMovie.Invoke("_root.api.Main.LeaderboardAnim.setLeaderboardText", gLangMngr.getString("$FR_LB_TOP_HG")); //battleroyale
		break;
	default:
		r3d_assert(false);
		break;
...
void FrontendWarZ::OnRequestLotterySuccess()
{
	...
	for (size_t i = 0; i < gUserProfile.LotLog_.size(); i++)
	{
		...
		char Bank[64] = { 0 };
		sprintf(Bank, "%s <font color = \"#68bb1f\">%d GD</font>", gLangMngr.getString("$FR_TOTAL_BANK"), gl.Bank);
		char strChance[32] = "";
...
void FrontendWarZ::OnMyServerListSuccess()
{
	...
	for(size_t i=0; i<gUserProfile.serverRent->serverList.size(); i++)
	{
		...
		vars[8].SetString(srv.ginfo.IsGameworld() ? gLangMngr.getString("MapType_Gameworld") : gLangMngr.getString("MapType_DeathMath"));
...
void FrontendWarZ::eventChangeOutfit(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	m_ChangeOutfit_newHair 	= args[1].GetUInt(); // hair system
	m_ChangeOutfit_newHead 	= args[2].GetUInt();
	m_ChangeOutfit_newBody 	= args[3].GetUInt();
	m_ChangeOutfit_newLegs 	= args[4].GetUInt();
	m_ChangeOutfit_newHero 	= args[5].GetUInt();
...
unsigned int WINAPI FrontendWarZ::as_ChangeOutfitThread(void* in_data)
{
	...
	int apiCode = gUserProfile.ApiCharChangeSkin(This->m_ChangeOutfit_newHair, This->m_ChangeOutfit_newHead, This->m_ChangeOutfit_newBody, This->m_ChangeOutfit_newLegs, This->m_ChangeOutfit_newHero);
	if(apiCode != 0)
	{
...
void FrontendWarZ::eventMarketplaceActive(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	gfxMovie.SetVariable("_root.api.Main.Marketplace.Marketplace.Tab9.visible", false);
	gfxMovie.SetVariable("_root.api.Main.Marketplace.Marketplace.Tab10.visible", false);
	gfxMovie.SetVariable("_root.api.Main.Marketplace.Marketplace.Tab11.visible", false);
	gfxMovie.SetVariable("_root.api.Main.Marketplace.Marketplace.Tab12.visible", false);
	r3d_assert(argCount==0);
	g_shown_marketplace_tutorial->SetBool(true);
	g_shown_frontend_tutorial->SetBool(true);
	writeGameOptionsFile();
	UpdateSkinIcons(gUserProfile.ProfileData.ArmorySlots[gUserProfile.SelectedCharID],true);
}
...
void FrontendWarZ::eventOpenURL(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	{
		ShellExecute(NULL, "open", "http://playafterlifegame.com/index.php?/topic/133636-regarding-bans-for-cheating/", "", NULL, SW_SHOW);
