...
void HUDARDBOX::eventBackpackGridSwap(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
...
void HUDARDBOX::eventBackpackFromInventory(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	PKT_C2S_ARDBOXItemARDBOXToBackpack_s n;
	n.ARDBOXID = CurrentARDBOXID;
	r3dscpy(n.AccessCodeS, CurrentAccessCodeString);
	n.ARDBOXInventoryID = inventoryID;
	...
	{
		if(ARDBOXContent[i].InventoryID == inventoryID)
		{
			n.dbg_InvItemID   = ARDBOXContent[i].itemID;
			n.dbg_InvQuantity = ARDBOXContent[i].quantity;
			break;
		}
	}
...
void HUDARDBOX::eventBackpackToInventory(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	PKT_C2S_ARDBOXItemBackpackToARDBOX_s n;
	n.ARDBOXID = CurrentARDBOXID;
	r3dscpy(n.AccessCodeS, CurrentAccessCodeString);
	n.SlotFrom     = gridFrom;
	n.Amount       = amount; 
...
void HUDARDBOX::eventReturnToGame(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	Deactivate();
}

void HUDARDBOX::eventChangeKeyCode(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	Deactivate();
	hudMain->showSafelockPin(false, CurrentARDBOXID, true);
}

void HUDARDBOX::eventPickupLockbox(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	if(NumItems > 0)
	{
		Scaleform::GFx::Value var[2];
		var[0].SetString(gLangMngr.getString("HUD_ARDBOX_ARDBOXShouldBeEmpty"));
		var[1].SetBoolean(true);
		gfxMovie.Invoke("_root.api.showInfoMsg", var, 2);
		return;
	}
	...
	PKT_C2S_ARDBOXPickup_s n;
	n.ARDBOXID  = CurrentARDBOXID;
	r3dscpy(n.AccessCodeS, CurrentAccessCodeString);
	p2pSendToHost(gClientLogic().localPlayer_, &n, sizeof(n));
}

void HUDARDBOX::setErrorMsg(const char* errMsg)
{
	if(isLockedUI)
	{
		isLockedUI = false;
...
bool HUDARDBOX::Init()
{
	if(!gfxMovie.Load("Data\\Interface\\Game_HUD_BOX.swf", false)) 
		return false;

#define MAKE_CALLBACK(FUNC) game_new r3dScaleformMovie::TGFxEICallback<HUDARDBOX>(this, &HUDARDBOX::FUNC)
...
void HUDARDBOX::reloadARDBOXInfo()
{
	...
	for(uint32_t i=0; i<NumItems; ++i)
	{
		if(ARDBOXContent[i].itemID == 0)
			continue;

		var[0].SetUInt(uint32_t(ARDBOXContent[i].InventoryID));
		var[1].SetUInt(ARDBOXContent[i].itemID);
		var[2].SetNumber(ARDBOXContent[i].quantity);
		var[3].SetNumber(ARDBOXContent[i].Var1);
		var[4].SetNumber(ARDBOXContent[i].Var2);
		bool isConsumable = false;
		{
			const WeaponConfig* wc = g_pWeaponArmory->getWeaponConfig(ARDBOXContent[i].itemID);
			if(wc && wc->category == storecat_UsableItem && wc->m_isConsumable)
				isConsumable = true;
		}
		var[5].SetBoolean(isConsumable);
		var[6].SetString(getAdditionalDescForItem(ARDBOXContent[i].itemID, ARDBOXContent[i].Var1, ARDBOXContent[i].Var2, ARDBOXContent[i].Var3));
		gfxMovie.Invoke("_root.api.addInventoryItem", var, 7);
	}
}

void HUDARDBOX::updateSurvivorTotalWeight()
{
	obj_Player* plr = gClientLogic().localPlayer_;
	r3d_assert(plr);
...
void HUDARDBOX::reloadBackpackInfo()
{
	updateSurvivorTotalWeight();
	// reset backpack
	{
