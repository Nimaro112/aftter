...
void HUDCrafting::eventCraftItem(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	if (gClientLogic().localPlayer_)
		SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/AM_Group/FrontEnd/Upgrade"),gClientLogic().localPlayer_->GetPosition());
}
...
{
	if(!gfxMovie.Load("Data\\Interface\\Game_HUD_Crafting.swf", false)) 
		return false;
...
void HUDCrafting::Activate()
{
	...
	}
	{
		...
		gfxMovie.Invoke("_root.api.setResWood", gUserProfile.ProfileData.ResWood);
		gfxMovie.Invoke("_root.api.setResStone", gUserProfile.ProfileData.ResStone);
		gfxMovie.Invoke("_root.api.setResMetal", gUserProfile.ProfileData.ResMetal);
