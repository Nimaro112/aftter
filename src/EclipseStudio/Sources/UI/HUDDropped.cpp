...
void HUDDropped::eventBackpackGridSwap(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
...
void HUDDropped::eventBackpackFromInventory(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	PKT_C2S_DroppedItemDroppedToBackpack_s n;
	n.droppedID = CurrentDroppedID;
	r3dscpy(n.AccessCodeS, CurrentAccessCodeString);
	n.DroppedInventoryID = inventoryID;
	...
	{
		if(DroppedContent[i].InventoryID == inventoryID)
		{
			n.dbg_InvItemID   = DroppedContent[i].itemID;
			n.dbg_InvQuantity = DroppedContent[i].quantity;
			break;
		}
	}
...
void HUDDropped::eventBackpackToInventory(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	
	Scaleform::GFx::Value var[2];
	var[0].SetString(gLangMngr.getString("$HUD_DROPPED"));
	var[1].SetBoolean(true);
	gfxMovie.Invoke("_root.api.showInfoMsg", var, 2);
	return;
	...
	PKT_C2S_DroppedItemBackpackToDropped_s n;
	n.droppedID = CurrentDroppedID;
	r3dscpy(n.AccessCodeS, CurrentAccessCodeString);
	n.SlotFrom     = gridFrom;
	n.Amount       = amount; 
...
void HUDDropped::eventReturnToGame(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	Deactivate();
}

void HUDDropped::eventChangeKeyCode(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	Deactivate();
	hudMain->showSafelockPin(false, CurrentDroppedID, true);
}

void HUDDropped::eventPickupLockbox(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	if(NumItems > 0)
	{
		Scaleform::GFx::Value var[2];
		var[0].SetString(gLangMngr.getString("HUD_Dropped_DroppedShouldBeEmpty"));
		var[1].SetBoolean(true);
		gfxMovie.Invoke("_root.api.showInfoMsg", var, 2);
		return;
	}
	...
	PKT_C2S_DroppedPickup_s n;
	n.droppedID  = CurrentDroppedID;
	r3dscpy(n.AccessCodeS, CurrentAccessCodeString);
	p2pSendToHost(gClientLogic().localPlayer_, &n, sizeof(n));
}

void HUDDropped::setErrorMsg(const char* errMsg)
{
	if(isLockedUI)
	{
		isLockedUI = false;
...
bool HUDDropped::Init()
{
	if(!gfxMovie.Load("Data\\Interface\\Game_HUD_Backpack.swf", false)) 
		return false;

#define MAKE_CALLBACK(FUNC) game_new r3dScaleformMovie::TGFxEICallback<HUDDropped>(this, &HUDDropped::FUNC)
...
void HUDDropped::Update()
{
}

void HUDDropped::Draw()
{
	gfxMovie.UpdateAndDraw();
}

void HUDDropped::Deactivate()
{
...
void HUDDropped::reloadDroppedInfo()
{
	...
	for(uint32_t i=0; i<NumItems; ++i)
	{
		if(DroppedContent[i].itemID == 0)
			continue;

		var[0].SetUInt(uint32_t(DroppedContent[i].InventoryID));
		var[1].SetUInt(DroppedContent[i].itemID);
		var[2].SetNumber(DroppedContent[i].quantity);
		var[3].SetNumber(DroppedContent[i].Var1);
		var[4].SetNumber(DroppedContent[i].Var2);
		bool isConsumable = false;
		{
			const WeaponConfig* wc = g_pWeaponArmory->getWeaponConfig(DroppedContent[i].itemID);
			if(wc && wc->category == storecat_UsableItem && wc->m_isConsumable)
				isConsumable = true;
		}
		var[5].SetBoolean(isConsumable);
		var[6].SetString(getAdditionalDescForItem(DroppedContent[i].itemID, DroppedContent[i].Var1, DroppedContent[i].Var2, DroppedContent[i].Var3));
		gfxMovie.Invoke("_root.api.addInventoryItem", var, 7);
	}
}

void HUDDropped::updateSurvivorTotalWeight()
{
	obj_Player* plr = gClientLogic().localPlayer_;
	r3d_assert(plr);
...
void HUDDropped::reloadBackpackInfo()
{
	updateSurvivorTotalWeight();
	// reset backpack
	{
