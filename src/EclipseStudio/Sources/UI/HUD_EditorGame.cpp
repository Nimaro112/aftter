...
void CreateDummyEditorPlayer()
{
	...
	pl->CurLoadout.HairIdx			= rand()%heroConf->getNumHairs(); // hair system
	...
	pl->CurLoadout.Items[wiCharDataFull::CHAR_LOADOUT_MASKGEAR].itemID	= SelectRandomGear( storecat_Mask );
	pl->CurLoadout.Items[wiCharDataFull::CHAR_LOADOUT_SCARFGEAR].itemID	= SelectRandomGear( storecat_Scarf );
	...
	pl->CurLoadout.startColdTime_ = 0;
	pl->CurLoadout.startDmgTime_ = 0;
...
obj_Player* EditorGameHUD::AddPlayer( int bControllable )
{
	...
	slot.HairIdx			= rand()%heroConf->getNumHairs(); // hair system
