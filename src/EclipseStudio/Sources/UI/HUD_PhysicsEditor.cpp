...
void PhysicsHUD::ProcessPhysicsEditor()
{
	...
	// CODE FOR AUTO UPDATING ALL PHYSICS MESH FILES :) /// see for PHYSICS_TYPE_MESH edited 0 1 2 3 uncknow 
	static int delay = 0;
	delay++;
	//////////// custom code to enable auto switch item viral 
	if (Keyboard->WasPressed(kbs4))
	{
		if (delay > 55 && idx != -1) ///add more delay 5bkp
		{
		...
		//////////// custom code to enable auto switch item viral
	if (Keyboard->WasPressed(kbs1)) ////////////// pres H  to Troggle Physics type to mesh 0 by now 
	{
		//Troggle Physics type 
		PhysicsObject->PhysicsConfig.type = PHYSICS_TYPE_MESH; /// MESH = 0 by now ;/

	}
	if (Keyboard->WasPressed(kbsM)) ////////////// Viral Press Mto create file and save pressA to chage file from list 
	{
		//1 create file
		PhysicsObject->PhysicsConfig.type = PHYSICS_TYPE_MESH; /// MESH = 0 by now ;/
		...
		else//////////////////(PhysicsObject->RecreatePhysicsObject && PhysicsObject->PhysicsConfig.type != PHYSICS_TYPE_UNKNOWN
			PhysicsObject->PhysicsObject = BasePhysicsObject::CreateStaticObject(PhysicsObject->PhysicsConfig, PhysicsObject);

		PhysicsObject->RecreatePhysicsObject = false;
		///2nd lap///// 2 save file  END

		//1 create file
	/*	PhysicsObject->PhysicsConfig.type = PHYSICS_TYPE_MESH; /// MESH = 0 by now ;/
		//PhysicsObject->RecreatePhysicsObject && PhysicsObject->PhysicsConfig.type != PHYSICS_TYPE_UNKNOWN;
		PhysicsObject->PhysicsObject = BasePhysicsObject::CreateStaticObject(PhysicsObject->PhysicsConfig, PhysicsObject);
		PhysicsObject->RecreatePhysicsObject = false;
		///// 2 save file 
	...
	if(PhysicsObject && PhysicsObject->GetObjectMesh())
	{
		...
		TypeNames.push_back("Mesh"); // changed for mesh first 

		//TypeNames.push_back("Unknown"); // changed for mesh first 
	...
	//	TypeNames.push_back("Unknown"); // changed for mesh first 
		...
		if (Keyboard->WasPressed(kbsG)) ////////////// Viral Press Mto create file and save pressA to chage file from list 
		{
			g_pPhysicsWorld->CookMesh(mesh);
			PhysicsObject->RecreatePhysicsObject = true;
		}
		/// last for a dinamic/static convex 
		if (Keyboard->WasPressed(kbs2)) ////////////// Viral Press Mto create file and save pressA to chage file from list 
		{
			...
			PhysicsObject->PhysicsConfig.isDynamic = true;
		...
		if(PhysicsObject->RecreatePhysicsObject && PhysicsObject->PhysicsConfig.type != PHYSICS_TYPE_UNKNOWN)
		{
			...
			/// server alpha custom coocker   "Create Physics Object"))
			if (Keyboard->IsPressed(kbsJ))
			{
				if (PhysicsObject->PhysicsObject)
					delete PhysicsObject->PhysicsObject;
				PhysicsObject->PhysicsObject = 0;
		...
		/// server alpha custom coocker  "Save physics data"))
		if (Keyboard->IsPressed(kbs3))
		{
			PhysicsObject->SavePhysicsData();
			PhysObjectCategories.at(current_obj_idx).ObjectsNames.at(current_obj_idx1).color = r3dColor(0, 255, 0);

		}			/// server alpha custom coocker  
