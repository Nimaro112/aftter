...
#include "ObjectsCode/Gameplay/obj_Stairs.h"
...
#include "UI\HUDDropped.h"
#include "UI\HUDARDBOX.h"
#include "UI\HUDTrade.h"
#include "UI\HUDRepair.h"
#include "UI\HUDMarket.h"
...
HUDDropped* hudDropped = NULL;
HUDARDBOX* hudARDBOX = NULL;
...
void TPSGameHUD_OnStartGame()
{
	...
	hudDropped = game_new HUDDropped();
	hudARDBOX = game_new HUDARDBOX();
...
void TPSGameHUD :: SetCameraPure ( r3dCamera &Cam)
{
	...
	// dead camera
	if(pl->bDead/* || pl->inWounded*/) // wounded disabled by now
	{
		...
		if(do_camera)
		{
			/*if(pl->inWounded) // wounded disabled by now
			{
	...
	/*if(pl->inWounded && hudAttm->isActive()) // wounded disabled by now
		hudAttm->Deactivate();*/
...
static void DrawMenus()
{
	...
	*/
	...
	if(!win::bSuspended && !hudMain->isChatInputActive() && !hudMain->isPlayersListVisible()) 
	{
		...
		if(showHudPause && !hudAttm->isActive() && !hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudDropped->isActive() && !hudARDBOX->isActive() && !hudTrade->isActive() && !hudTrade->isActive() && !hudMarket->isActive() && !hudRepair->isActive() && !hudCraft->isActive())
		{
		...
		if((Keyboard->WasPressed(kbsEsc)) && hudMarket->isActive())
			hudMarket->Deactivate();

	if(gUserProfile.ProfileData.isDevAccount) // CRAFTING SYSTEM KEYBOARD BLOCK ONLY FOR NORMAL PLAYER
	{
		bool showCraft = InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_SHOW_CRAFT_MENU);
		if(showCraft && !hudAttm->isActive() && !hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudDropped->isActive() && !hudARDBOX->isActive() && !hudTrade->isActive() && !hudMarket->isActive() && !hudRepair->isActive() && !hudPause->isActive())
		{
...
//#ifdef ENABLE_INGAME_STORE
	...
	if(gClientLogic().m_gameInfo.mapId==GBGameInfo::MAPID_AM_Devmap || gClientLogic().m_gameInfo.mapId==GBGameInfo::MAPID_AM_Compound || gClientLogic().m_gameInfo.mapId==GBGameInfo::MAPID_AM_ConventionC
		 || gClientLogic().m_gameInfo.mapId==GBGameInfo::MAPID_AM_RentServer01 || gClientLogic().m_gameInfo.mapId==GBGameInfo::MAPID_AM_RentServer02)
    {
		if(showStore && !hudAttm->isActive() && !hudVault->isActive() && !hudSafelock->isActive() && !hudDropped->isActive() && !hudARDBOX->isActive() && !hudTrade->isActive() && !hudRepair->isActive())
		{
...
//#endif
		...
		if((Keyboard->WasPressed(kbsEsc)) && hudDropped->isActive())
			hudDropped->Deactivate();

		if((Keyboard->WasPressed(kbsEsc)) && hudARDBOX->isActive())
			hudARDBOX->Deactivate();
		...
		if( pl && !hudCraft->isActive() && !hudAttm->isActive() && !hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudDropped->isActive() && !hudARDBOX->isActive() && !hudTrade->isActive() && !hudMarket->isActive() && !hudRepair->isActive() && !hudPause->isActive() )
		{
	...
	if(hudMarket->isActive())
	{
		r3dMouse::Show(); // make sure that mouse is visible

		R3DPROFILE_START( "hudMarket->" );

		hudMarket->Update();
		hudMarket->Draw();

		R3DPROFILE_END( "hudMarket->" );
	...
	if(hudDropped->isActive())
	{
		r3dMouse::Show(); // make sure that mouse is visible

		R3DPROFILE_START( "hudDropped->" );

		hudDropped->Update();
		hudDropped->Draw();

		R3DPROFILE_END( "hudDropped->" );

		return;
	}

	if(hudARDBOX->isActive())
	{
		r3dMouse::Show(); // make sure that mouse is visible

		R3DPROFILE_START( "hudARDBOX->" );

		hudARDBOX->Update();
		hudARDBOX->Draw();

		R3DPROFILE_END( "hudARDBOX->" );
	...
	else
	{
		...
		if(ChatWindowSwitch && hudMain && !hudMain->isChatInputActive() && !hudMain->isPlayersListVisible()/* && !hudMain->isVoiceCmdVisible()*/ && r_render_in_game_HUD->GetBool() && pl->uavViewActive_==false)
		{
		...
		if(hudMain && !hudMain->isChatInputActive() && !hudMain->isPlayersListVisible()/* && !hudMain->isVoiceCmdVisible()*/)
		{
		...
		if (hudMain && !hudMain->isChatInputActive())
		{
			...
			/*bool altPressed = InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_SHOW_VOICECMD);
			bool altReleased =  InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_SHOW_VOICECMD);*/
			bool escPressed = Keyboard->WasPressed(kbsEsc);
			...
			/*if (altPressed && !hudMain->isVoiceCmdVisible())
			{
				hudMain->showVoiceCmd(1);
			}
			else if ((altReleased || escPressed) && hudMain->isVoiceCmdVisible())
			{
				hudMain->showVoiceCmd(0);
			}*/
		}
...

		{
			...
			if(!pl->bDead && (r3dGetTime() - pl->TimeOfLastRespawn)>300.0f && !hudAttm->isActive() && !hudPause->isActive() && !hudActionUI->isActive() && 
				!hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudDropped->isActive() && !hudARDBOX->isActive() && !hudTrade->isActive() && !hudMarket->isActive() && !hudRepair->isActive() && !hudCraft->isActive())
			{
...
bool SetCameraPlayerVehicle(const obj_Player* pl, r3dCamera &Cam)
{
	...
	if (gClientLogic().localPlayer_ && gClientLogic().localPlayer_->IsInVehicle())
	{
		...
		else
		{
			...
			if (InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_CONSTRUCTOR) && gClientLogic().localPlayer_->seatPosition == 0 && !(hudPause && hudPause->isActive()) && !(hudMain && hudMain->isPlayersListVisible())/* && !(hudMain && hudMain->isVoiceCmdVisible())*/ && !(hudMain && hudMain->isChatInputActive()))
			{
			...
			if (InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_SWITCH_FPS_TPS) && !(hudPause && hudPause->isActive()) && !(hudMain && hudMain->isPlayersListVisible())/* && !(hudMain && hudMain->isVoiceCmdVisible())*/ && !(hudMain && hudMain->isChatInputActive()))
			{
...
void ProcessPlayerMovement(obj_Player* pl, bool editor_debug )
{
	...
	if( !g_bEditMode )
	{
		...
		if (pl->IsInVehicle())
		{
			...
			//if(hudMain->isVoiceCmdVisible()) return;
			if(hudPause->isActive()) return;
			if(hudVault->isActive()) return;
			if(hudMarket->isActive()) return;
			if(hudRepair->isActive()) return;
	...
	if(InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_SWITCH_FPS_TPS) && !(hudAttm && hudAttm->isActive()) && !(hudMain && (hudMain->isChatInputActive() || hudMain->isPlayersListVisible()/* || hudMain->isVoiceCmdVisible()*/)) && !Mouse->GetMouseVisibility() && !stillPlayingProningAnim)
	{
		pl->switchFPS_TPS();
	}
	...
	{
		if(Keyboard->WasPressed(kbsLeftShift) && !(hudVault && hudVault->isActive()) && 
			!(hudCraft && hudCraft->isActive())&& !(hudMarket && hudMarket->isActive())&& !(hudRepair && hudRepair->isActive()) && !(hudStore && hudStore->isActive()) && 
			!(hudDropped && hudDropped->isActive()) && !(hudARDBOX && hudARDBOX->isActive()) &&
		...
		if(wpn->getItemID() == WeaponConfig::ITEMID_SpyDrone && !pl->uavViewActive_) // UAV
		{
	...
	if(!swimming)
	{
		...
		{
			disablePlayerMovement = false;// default is TRUE
			disablePlayerRotation = false;// default is TRUE
		}
	...
	
	{
		...
		if(!disablePlayerMovement)
		{
			...
			if(aiming || pl->m_isHoldingBreath /*|| pl->CurLoadout.GameFlags & wiCharDataFull::GAMEFLAG_NearRadBox*/) // cannot spring and aim. also, in default key binding spring and hold breath are on the same key
				shiftWasPressed = 0;
			...
			else
			{
				...
				if(g_AutoRunToggled)
				{
					if(InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_MOVE_FORWARD) || InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_MOVE_BACKWARD))
			...
			if(gUserProfile.ProfileData.isDevAccount && Keyboard->IsPressed(kbsV))			
				accelaration *= 8.0f;			
#else
			if(g_bEditMode && Keyboard->IsPressed(kbsV))			
				accelaration *= 8.0f;			
			else if(!g_bEditMode && Keyboard->IsPressed(kbsV))			
				accelaration *= 8.0f;			
#endif
			if(gClientLogic().localPlayer_->CurLoadout.GameFlags & wiCharDataFull::GAMEFLAG_NearStairs) // ladders system
			{
				if(InputMappingMngr->isPressed(r3dInputMappingMngr::KS_JUMP))
				{                    
					pl->JumpVelocity  = 15;
				}
			}
			...
			// 			accelaration *= 0.1f; // 20% slow down
...
////================ Taunt Through numeric keys ===============
	...
	if(Keyboard->IsPressed(kbsNumericPgDn)) // Dancing
	{
		pl->StartDance();
	}
	...
	if(Keyboard->IsPressed(kbsNumericHome)) // backflip
	{
		pl->StartBackflip();
	}*/

/////////////////////// Character Voice Command ////////////////////
	/*if( Keyboard->IsPressed( kbsB ) && Keyboard->WasPressed( kbs1 ) )
	{
		pl->CmdVcOne();
	}
	if( Keyboard->IsPressed( kbsB ) && Keyboard->WasPressed( kbs2 ) )
	{
		pl->CmdVcTwo();
	}
	if( Keyboard->IsPressed( kbsB ) && Keyboard->WasPressed( kbs3 ) )
	{
		pl->CmdVcThree();
	}
	if( Keyboard->IsPressed( kbsB ) && Keyboard->WasPressed( kbs4 ) )
	{
		pl->CmdVcFour();
	}
	if( Keyboard->IsPressed( kbsB ) && Keyboard->WasPressed( kbs5 ) )
	{
		pl->CmdVcFive();
	}
	if( Keyboard->IsPressed( kbsB ) && Keyboard->WasPressed( kbs6 ) )
	{
		pl->CmdVcSix();
	}
	if( Keyboard->IsPressed( kbsB ) && Keyboard->WasPressed( kbs7 ) )
	{
		pl->CmdVcSeven();
	}
	if( Keyboard->IsPressed( kbsB ) && Keyboard->WasPressed( kbs8 ) )
	{
		pl->CmdVcEight();
	}
	if( Keyboard->IsPressed( kbsB ) && Keyboard->WasPressed( kbs9 ) )
	{
		pl->CmdVcNine();
	}*/
/////////////////////// Character Voice Command ////////////////////
