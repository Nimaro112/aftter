...
REG_VAR( d_disable_vests_draw,  	false,			0 );		// MMZ. Disable vest drawing
...
REG_VAR( d_debug_show_dropped_owner, false,			0 );
REG_VAR( d_debug_show_ARDBOX_owner, false,			0 );
...
#define R3D_MAX_GRASS_PATCH_DISTANCE_COEF 3.0f

REG_VAR( r_grass_view_dist,			86.f,			0 );
...
REG_VAR( r_show_ai_npc_stats		, false		, 0 );
...
REG_VAR( r_instanced_particles		, false		, 0 );
...
REG_VAR( d_dev_show_player_names		,false  , VF_SAVE ); // ESP DEV "/esp"
...
REG_VAR( r_editor_move_speed,		15.0f		, 0);
...
REG_VAR( g_shown_frontend_tutorial, false, VF_SAVE );
...
REG_VAR_C( g_mouse_sensitivity		, 1.f		, 0.1f, 15.0f, VF_SAVE | VF_CONSTRAINT);
...
REG_VAR( r_fastload				, true			, VF_SAVE );
...
//REG_VAR_C( r_postprocess_quality	, 1			, 1, 3, VF_SAVE | VF_CONSTRAINT );
REG_VAR_C( r_postprocess_quality	, 1			, 1, 2, VF_SAVE | VF_CONSTRAINT );
REG_VAR_C( r_ssao_quality			, 3			, 1, 5, VF_SAVE | VF_CONSTRAINT );
...
REG_VAR( g_serverip,			"25.91.238.22",	0 );
REG_VAR( g_api_ip,				"25.91.238.22",	0);
...
REG_VAR( g_ai_npc_update_radius,		512,			0 );
...
REG_VAR( g_enable_AI_NPC_sprint,		true,			0 );
