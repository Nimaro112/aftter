...
int CSoundSystem::Close()
{
	...
	if (FMODEventSystem)
	{
		FMODEventSystem->unloadFSB("Sound_Vault.fsb", 0);
		m_SoundBank00->release();
		FMODEventSystem->unloadFSB("Game_Music.fsb", 0);
		m_SoundBank_Streaming->release();
		FMODEventSystem->unloadFSB("AutomaticWeapons.fsb", 0);
		m_SoundBank_Weapons->release();

		FMODEventSystem->unloadFSB("Game_Sounds.fsb", 0);
...
int CSoundSystem::LoadSoundEffects(const char *basedir, const char *fname)
{
	...
	rv = FMODSystem->createSound("Data/Sounds/Sound_Vault.fsb", FMOD_CREATECOMPRESSEDSAMPLE, 0, &m_SoundBank00); SND_ERR_CHK(rv);
	rv = FMODEventSystem->preloadFSB("Sound_Vault.fsb", 0, m_SoundBank00); SND_ERR_CHK(rv);

	rv = FMODSystem->createSound("Data/Sounds/Game_Music.fsb", FMOD_CREATECOMPRESSEDSAMPLE, 0, &m_SoundBank_Streaming); SND_ERR_CHK(rv);
	rv = FMODEventSystem->preloadFSB("Game_Music.fsb", 0, m_SoundBank_Streaming); SND_ERR_CHK(rv);
	...
	rv = FMODSystem->createSound("Data/Sounds/Game_Sounds.fsb", FMOD_CREATECOMPRESSEDSAMPLE, 0, &m_WarZ_SoundBank); SND_ERR_CHK(rv);
	rv = FMODEventSystem->preloadFSB("Game_Sounds.fsb", 0, m_WarZ_SoundBank); SND_ERR_CHK(rv);
