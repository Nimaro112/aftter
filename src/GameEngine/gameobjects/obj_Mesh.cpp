...
void MeshGameObject::AppendRenderablesInstanced( RenderArray ( & render_arrays  )[ rsCount ], const r3dCamera& Cam )
{
	...
	if(r_rc_highlight_radius->GetFloat() > 0.0f)
	{
		...
		if((gCam - GetPosition()).LengthSq() <= rc_R )
		{
			...
			if( currentInstancingMesh->HasMaterialOfType( "Concrete_Resource" ) || currentInstancingMesh->HasMaterialOfType( "Wood_Resources" ) || currentInstancingMesh->HasMaterialOfType( "Metal_Resources" ) || currentInstancingMesh->HasMaterialOfType( "Meteor_Resources" ) )
			{
...
void MeshGameObject::AppendRenderables( RenderArray ( & render_arrays  )[ rsCount ], const r3dCamera& Cam )
{
	...
	if(r_rc_highlight_radius->GetFloat() > 0.0f)
	{
		...
		if((gCam - GetPosition()).LengthSq() <= rc_R )
		{
			if( TargetLODSet[ meshLodIndex ]->HasMaterialOfType( "Concrete_Resource" ) || TargetLODSet[ meshLodIndex ]->HasMaterialOfType( "Wood_Resources" ) || TargetLODSet[ meshLodIndex ]->HasMaterialOfType( "Metal_Resources" ) || TargetLODSet[ meshLodIndex ]->HasMaterialOfType( "Meteor_Resources" ) )
			{
				MeshObjDeferredHighlightRenderable rend;

				rend.Init( MeshGameObject::GetObjectLodMesh(), this, r3dColor::gold.GetPacked()  );
				rend.SortValue = 0;
				rend.DoExtrude = 0;
