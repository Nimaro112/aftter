...
{	
	...
	if (CreateParams.vehicleType == VEHICLETYPE_COPCAR)
...
{	
	...
	if (m->FileName == "data/objectsdepot/g3_vehicles/drivable_cop_car.sco")
...
void obj_Vehicle::CheckIsStuck()
{
	...
	if (!isCurrentlyStuck && isStuckInFront && isStuckInBack)
	{
		...
		if (r3dGetTime() > lastStuckMessageTime + stuckMessageWait)
		{
			...
			hudMain->addChatMessage(0, "[SERVER]", "If you are stuck, exit your vehicle, look at it and press the UNSTUCK key(F5).", 2);
...
void obj_Vehicle::CheckInput()
{
	if(NetworkLocal)
	{
		...
		if( hudMain && ( hudMain->isChatInputActive() || hudMain->isPlayersListVisible()/* || hudMain->isVoiceCmdVisible()*/ ) ) return;
...
void obj_Vehicle::OnNetPacket(const PKT_S2C_VehicleUnstuck_s& n)
{
	if (n.isSuccess)
	{
		...
		SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/Vehicles/Unstuck/Lock_Pick_Success"), GetPosition());
	}
	else
	{
		SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/Vehicles/Unstuck/Lock_Pick_Failure"), GetPosition());
	}
}
