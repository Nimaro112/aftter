...
CUpdater::CUpdater()
{
  ...
  InitializeCriticalSection(&csSlider_);
...
{
/* //@DISABLED, no support on AfterLife backend yet
	// must be logged in
...
void CUpdater::WaitReadyToPlay()
{
  ...
  // infinite loop, updating server status from news thread
	while(true)
	{
		::Sleep(1 * 1000);
		IsServerOnline();
		if(status_ == STATUS_NeedLogin) {
			sprintf(updMsg1_, "Log in to launch game");
		} else if(status_ == STATUS_Updating) {
			sprintf(updMsg1_, "Log in successful, click Play Game to launch Game");
		}
	}
...
void CUpdater::ParseSliderNode(pugi::xml_node xmlNode, std::vector<slider_s>& slider)
{
	slider.clear();
	...
	while(!xmlNode.empty())
	{
		...
		n.image_     = xmlNode.attribute("img").value();
		slider.push_back(n);
...
int CUpdater::GetNews()
{
  ...
  if(sliderStatus_ == 0) // download slider data only once
  {
    ParseSliderNode(xmlInfo.child("slider"), sliderData_);

    sliderStatus_ = 1;
    
    // and start picture download thread for slider
    sliderThread_ = (HANDLE)_beginthreadex(
      NULL,
      0,
      &CUpdater_SliderThreadEntry,
      (void *)this,		// argument
      0,			// creation flags
      NULL			// thread ID
    );
...
void CUpdater::SliderThreadEntry()
{
  try
  {
    for(size_t i=0; i<sliderData_.size(); i++)
    {
      slider_s& r = sliderData_[i];
      ...
      if(!http.Get(url, r.imgData_)) {
        ...
        r3dOutToLog("Slider: failed to download %s\n", url);
        r.imgStatus_ = 1;
        continue;
      }
      
      //r3dOutToLog("Slider: got %s\n", url);
      r.imgStatus_ = 2;
    }
  ...
  {
    MessageBox(NULL, "something wrong happened in Slider", "", MB_OK);
  }
...
static unsigned int __stdcall CUpdater_SliderThreadEntry(LPVOID in)
{
  CUpdater* impl = (CUpdater*)in;
  impl->SliderThreadEntry();
  return 0;
}
...
void CUpdater::ProcessLoginResults()
{
	...
	int res = MessageBox(NULL, "Banned: This account has been investigated and permanently closed for violations of the Terms of Use", "Login", MB_OK | MB_ICONSTOP);
        if(res == IDOK)
	{
	  // THAI ShellExecute(NULL, "open", "http://www.AfterLifemmo.com/tos.html", "", NULL, SW_SHOW);
	  // THAI ShellExecute(NULL, "open", "http://forums.AfterLifemmo.com/index.php?/topic/133636-regarding-bans-for-cheating/", "", NULL, SW_SHOW);
	}
        break;
      }
      ...
      case CLoginHelper::ANS_SteamDoNotOwnGame:
        loginErrMsg_ = "";
		MessageBox(NULL, "You've started game under a steam account, but that account does not have AfterLife MMO game ownership", "No game ownership on Steam", MB_OK | MB_ICONEXCLAMATION);
        break;
        
      case CLoginHelper::ANS_Unknown:
        loginErrMsg_ = "Unknown status";
        MessageBox(NULL, "Unknown account status, please contact support.", "Login", MB_OK | MB_ICONEXCLAMATION);
        break;
...
void CUpdater::CreateAccThreadEntry()
{
  try
  {
    ...
    switch(code)
    {
      ...
      case 7:
        MessageBox(NULL, "You already have created HTS account linked with this Steam account.", "Create Account", MB_OK | MB_ICONSTOP);
        break;

      case 0xB:
        MessageBox(NULL, "Please use your existing HTS account to play game under Steam.", "Converted Account", MB_OK | MB_ICONSTOP);
        break;
    }
  }
  catch(char* err)
...
void CUpdater::DoCreateAccount()
{
  ...
  if(loginThread_ = (HANDLE)_beginthreadex){
  MessageBox(NULL, "Successful registration! Please restart the launcher.", "Restart required", MB_OK | MB_ICONINFORMATION);
  g_bExit = true;
  }
  return;
}
