...
class CUpdater
{
  ...
  public:	
	...
	std::vector<slider_s> sliderData_;
	volatile DWORD 	sliderStatus_;
	CRITICAL_SECTION csSlider_;
  ...
  private:
	...
	void         ParseSliderNode(pugi::xml_node xmlNode, std::vector<slider_s>& slider);
	void		 FilterSurveyLinks();
	HANDLE		newsThread_;
	friend static unsigned int __stdcall CUpdater_SliderThreadEntry(LPVOID in);
	void		SliderThreadEntry();
	HANDLE		sliderThread_;
