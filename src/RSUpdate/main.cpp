...
static int fake_SetMode(int XRes, int YRes)
{
  ...
  _r3dSystemFont = new CD3DFont(ig, "Titillium", 10, D3DFONT_BOLD | D3DFONT_FILTERED | D3DFONT_SKIPGLYPH);
  _r3dSystemFont->CreateSystemFont();
  
  extern int r3d_CD3DFont_Quality;
  r3d_CD3DFont_Quality = 5; // set to CLEARTYPE_QUALITY

  g_font1 = new CD3DFont(ig, "Titillium", 10, D3DFONT_FILTERED | D3DFONT_SKIPGLYPH);
  g_font1->CreateSystemFont();
  g_font2 = new CD3DFont(ig, "Titillium", 9.5, D3DFONT_FILTERED | D3DFONT_SKIPGLYPH);
  g_font2->CreateSystemFont();
  g_font3 = new CD3DFont(ig, "Titillium", 8.5, D3DFONT_FILTERED | D3DFONT_SKIPGLYPH);
...
{
  g_bkgLogin    = new r3dTexImage2D(IDR_IMAGE_BACK_LOGIN, 1024, 660);
  g_bkgUpdate   = new r3dTexImage2D(IDR_IMAGE_BACK_UPDATE, 1024, 660);
  g_bkgRegister = new r3dTexImage2D(IDR_IMAGE_BACK_REGISTER, 1024, 660);
  g_bkgSerialCheck = new r3dTexImage2D(IDR_IMAGE_BACK_SERIALCHECK, 1024, 660);
  g_bkgStarting = new r3dTexImage2D(IDR_IMAGE_BACK_STARTING, 1024, 660);
  g_bkgExpired = new r3dTexImage2D(IDR_IMAGE_BACK_EXPIRED, 1024, 660);
  g_imPB[0] = new r3dTexImage2D(IDR_IMAGE_PB_LEFT, 3, 4);
  g_imPB[1] = new r3dTexImage2D(IDR_IMAGE_PB_CENTER, 1020, 4);
  ...
  g_imBtnTopMenu[0] = new r3dTexImage2D(IDR_IMAGE_BTN_MYACC_HOVER, 1, 1);//256, 64); // NUL

  g_imBtnTopMenu[1] = new r3dTexImage2D(IDR_IMAGE_BTN_FORUMS_HOVER, 89, 30); // HELP

  g_imBtnTopMenu[2] = new r3dTexImage2D(IDR_IMAGE_BTN_SUPPORT_HOVER, 96, 30); // BUY GC

  g_imBtnTopMenu[3] = new r3dTexImage2D(IDR_IMAGE_BTN_YOUTUBE_HOVER, 30, 30);
  g_imBtnTopMenu[4] = new r3dTexImage2D(IDR_IMAGE_BTN_FACEBOOK_HOVER, 30, 30);
  g_imBtnTopMenu[5] = new r3dTexImage2D(IDR_IMAGE_BTN_TWITTER_HOVER, 30, 30);
  
  g_imBtnPg[0] = new r3dTexImage2D(IDR_IMAGE_BTN_PG_ON,    256, 128);
  g_imBtnPg[1] = new r3dTexImage2D(IDR_IMAGE_BTN_PG_HOVER, 256, 128);

  g_imBtnPg2[0] = new r3dTexImage2D(IDR_IMAGE_BTN_PG_ON,    256, 128);
  g_imBtnPg2[1] = new r3dTexImage2D(IDR_IMAGE_BTN_PG_HOVER, 256, 128);
...
void game::Init(void)
{
  ...
  char title[512];
  sprintf(title, "AfterLife MMO Launcher %s (%s)", UPDATER_VERSION, UPDATER_BUILD);
...
static void drawProgressBar(CUpdater& updater)
{
  ...
  g_font3->PrintF(10, 620, r3dColor(255, 255, 255), "Open Beta: v%s %s", UPDATER_VERSION,UPDATER_VERSION_SUFFIX); //Open Beta Beta Version
  g_font2->DrawText(0, 630, 1024, 25, r3dColor(255, 255, 255), updater.updMsg1_, D3DFONT_CENTERED);
...
static void drawCloseButton() //785
{
  static r3dRECT g_rCloseBtn = {971, 0, 50, 50};

  // close button
  r3dColor clr(255, 255, 255);
  if(insideRect(g_rCloseBtn, g_mx, g_my)) {
    g_imBtnClose->draw(971, 0, 64, 64, clr);
    if(g_mb) {
      g_bExit = true;
    }
  }
...
{
  static r3dRECT g_rRegisterBtn  = {171, 503, 256, 64};
...
static void drawTimeExpiredButtons(CUpdater& updater)
{
  ...
  if(insideRect(g_rGetNewKeyBtn, g_mx, g_my)) 
  {
    ...
    if(g_mb) 
    {
      ShellExecute(NULL, "open", "http://playafterlifegame.com/#getinfo", "", NULL, SW_SHOW);
    }
  }
...
{
  static r3dRECT g_rValidateBtn   = {92, 454, 101, 31};
  static r3dRECT g_rGetSerialBtn  = {260, 455, 188, 31};
  ...
  // some temp warning
  {
    ...
    g_font1->PrintF(143, 375, clr, "You can paste (Control-V) your serial below");
  ...
  if(insideRect(g_rGetSerialBtn, g_mx, g_my)) 
  {
    ...
    if(g_mb) 
    {
      ShellExecute(NULL, "open", "http://playafterlifegame.com/newserial/", "", NULL, SW_SHOW);
    }
  }
...
{
  static r3dRECT g_rLoginBtn  = {135, 478, 87, 35};
  static r3dRECT g_rRegisterNewBtn = {270, 478, 184, 31};
  static r3dRECT g_rLoginEdit = {200, 368, 424, 24};
  static r3dRECT g_rPwdEdit   = {200, 420, 424, 24};
  r3dColor clr(255, 255, 255);
  
  if(drawUrl(340, 448, "Forgot Password?")) 
  {
    ShellExecute(NULL, "open", "https://playafterlifegame.com/forgotpassword/", "", NULL, SW_SHOW);
  }
  ...
  if(*updater.gUserProfile.username && *updater.gUserProfile.passwd && insideRect(g_rLoginBtn, g_mx, g_my)) {
    g_imBtnLogin->draw(135, 478, 87, 35, clr);
    if(g_mb) {
      updater.DoLogin();
	  //ShellExecute(NULL, "open", "http://forum.playafterlifegame.com/index.php?/topic/12-guide-beginners-guide-1/", "", NULL, SW_SHOW);
    }
  }
  ...
  {
    g_imBtnRegisterNew->draw(270, 478, 184, 31, clr);
    if(g_mb) {
    if(eulaShowDialog(0) != IDOK) { // EULA term
      return;
      }
  ...
  if(updater.loginErrMsg_[0]) {
    ...
    g_font1->DrawText(bx, by, 331, 25, r3dColor(255, 255, 255), updater.loginErrMsg_, D3DFONT_CENTERED);
  }
  ...
  int key;
  key = editTextField(g_rLoginEdit.left, g_rLoginEdit.top, updater.gUserProfile.username, focusId == 0, 0, 32);
  if(key == 9)  focusId = 1;
...
static void executeTopMenuButton(const CUpdater& updater, int btnIdx)
{
  ...
  switch(btnIdx)
  {
    ...
    {
      char url[1024];
      sprintf(url, "http://playafterlifemmo.ddns.net/gamecp/index.php"); // /?WzLogin=%s", token);
    ...
    case 1: // forums // HELP
    {
      ShellExecute(NULL, "open", "http://community.playafterlifegame.com/", "", NULL, SW_SHOW);
      break;
    }

    case 2: // support // BUY GC
    {
      ShellExecute(NULL, "open", "http://playafterlifemmo.ddns.net/gamecp/index.php", "", NULL, SW_SHOW);
      break;
    }
    
    case 3: // youtube
    {
      ShellExecute(NULL, "open", "https://www.youtube.com/channel/UCOV7O8DV8W90-mCQt8IjPVw/videos?view_as=subscriber", "", NULL, SW_SHOW);
      break;
    }
    
    case 4: // facebook
    {
      ShellExecute(NULL, "open", "https://www.facebook.com/afterlifemmo", "", NULL, SW_SHOW);
      break;
    }
    
    case 5: // twitter - TS3
    {
      ShellExecute(NULL, "open", "ts3server://ts.playafterlifemmo.com", "", NULL, SW_SHOW);
      break;
    }
  }
...
{
  static r3dRECT g_rPlayBtn  =  {399, 575, 202, 53}; //{506, 521, 244, 61};
  static r3dRECT g_rPlayBtn2  = {399, 575, 202, 53};
  static r3dRECT g_rTopBtn[6] = {
    {1, 1, 1, 1}, // my account? NULL
    {554, 220,  89, 30}, // forums ? HELP
    {679, 220,  96, 30}, //	support ? BUY GC
    {913, 222,  30, 30}, // topcins ts3
    {875, 222,  30, 30}, // topcins facebook
    {838, 222,  30, 30} // topcins youtube
  };
  ...
  if(updater.result_ == CUpdater::RES_PLAY && updater.IsServerOnline())
  {
    if(insideRect(g_rPlayBtn, g_mx, g_my)) {
      g_imBtnPg[1]->draw(399, 575, 202, 53, clr); //(489, 484, 276, 127, clr);
    } else {
      g_imBtnPg[0]->draw(399, 575, 202, 53, clr);
    }
  ...
  // top menu buttons
  for(int btnIdx=0; btnIdx<6; btnIdx++) // for(int btnIdx=0; btnIdx<6; btnIdx++)
  {
...
static void drawNewsSection(float x, float y, const std::vector<CUpdater::news_s>& news, size_t maxLines)
{
  ...
  for(size_t i=0; i<news.size() && i < maxLines; i++) 
  {
    ...
    r3dRECT r = {600, y-40, 1024, 25};
    r3dColor clr = r3dColor(255, 255, 255);
    
    // selection and underline
    if(insideRect(r, g_mx, g_my)) {
      clr = r3dColor(250, 216, 0);
      //r3dDrawBox2D(x, y + (float)ts.cy, (float)ts.cx, 1, clr);
      if(g_mb) {
    ...
    g_font2->DrawText(255, y-40, 1024, 25, clr, newsText, D3DFONT_CENTERED);
    y += 20;
  }
...
static void drawNews(CUpdater& updater)
{
  ...
  drawNewsSection(43, 366, updater.newsData_, 10);
...
static void drawServerStatus(const CUpdater& updater)
{
  ...
  if(updater.newsStatus_ == 0) {
    g_font3->PrintF(10, y, r3dColor(255, 255, 255), "Retrieving Server Status");
    return;
  }

  if(updater.newsStatus_ == 1 || updater.serverStatus_.size() == 0) {
    g_font3->PrintF(10, y, r3dColor(255, 255, 255), "Server Status Unavailable");
    return;
  }
  
  g_font3->PrintF(10, y, r3dColor(255, 255, 255), "Server Status: ");
  const char* status = updater.serverStatus_.c_str();
  if(stricmp(status, "ONLINE") == 0)
    g_font3->PrintF(80, y, r3dColor(31, 255, 57), "ONLINE");
  else if(stricmp(status, "OFFLINE") == 0)
    g_font3->PrintF(80, y, r3dColor(255, 31, 31), "OFFLINE");
  else
    g_font3->PrintF(80, y, r3dColor(255, 127, 39), status);
    
  return;
}

static void sliderCreateTexture(CUpdater::slider_s& rot)
{
	// create slider texture if we downloaded data
	if(rot.imgStatus_ == 2) 
	{
		...
		{
			r3dOutToLog("Slider: failed to create texture from %s\n", rot.image_);
		}
...
static void drawSlider(CUpdater& updater)
{
	static r3dRECT rRotImg = {533, 40, 466, 154};


	if(updater.sliderStatus_ != 1)
		return;
	if(updater.sliderData_.size() == 0)
		return;
	...
	if(r3dGetTime() > imgStartTime + TIME_IN_IMAGE) 
	{
		...
		size_t newIdx = (rotIdx + 1) % updater.sliderData_.size();
		
		if(updater.sliderData_[newIdx].imgStatus_ == 0)
			return;

		if(newIdx != rotIdx)
		{
			sliderCreateTexture(updater.sliderData_[newIdx]);

			// switch to new image
			if(rotIdx >= 0) 
			prevTex    = updater.sliderData_[rotIdx].tex_;
			rotIdx       = newIdx;
			imgStartTime = r3dGetTime();
		}
	}

	r3d_assert(rotIdx >= 0 && rotIdx < (int)updater.sliderData_.size());
	CUpdater::slider_s& rot = updater.sliderData_[rotIdx]; 
  
	if(rot.tex_ != NULL) 
	{
		if(prevTex) 
		r3dDrawBox2D(533, 40, 466, 154, r3dColor(255, 255, 255), prevTex);


		float a = 255.0f * R3D_MIN(1.0f, (r3dGetTime()-imgStartTime) / TIME_TO_FADEIN);
		r3dDrawBox2D(533, 40, 466, 154, r3dColor(255, 255, 255, (int)a), rot.tex_);
	} 
	else 
	{
		r3dDrawBox2D(533, 40, 466, 154, r3dColor(0, 0, 0, 100), NULL);
	}
...
*/
 ...
 //sprintf(GAME_START_PARAM, "-226700 -WOUpdatedOk %s -WOLogin \"%s\" -steam", __r3dCmdLine, token);
