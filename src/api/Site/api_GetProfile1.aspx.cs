...
public partial class api_GetProfile1 : WOApiWebPage
{
    void AddChars(ref StringBuilder xml, bool FromServer)
    {
        ...
        while (reader.Read())
        {
            ...
            xml.Append(xml_attr("HairIdx", reader["HairIdx"])); // hair system
    ...
    void AddWeaponSkins(ref StringBuilder xml)
    {
        // move to next select, where data should be
        reader.NextResult();

        xml.Append("<weaponskins>\n");
        while (reader.Read())
        {
            int WeaponItemID = Convert.ToInt32(reader["WeaponItemID"]);
            int Skins = Convert.ToInt32(reader["Skins"]);
            int Selected = Convert.ToInt32(reader["Selected"]);

            xml.Append("<Skn ");
            xml.Append(xml_attr("itemID", WeaponItemID));
            xml.Append(xml_attr("Skins", Skins));
            xml.Append(xml_attr("Selected", Selected));
            xml.Append("/>");
        }
